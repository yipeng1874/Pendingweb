...
async function migrate() {
  ...
  `);
    ...
    try {
        await prisma.$executeRawUnsafe(`ALTER TABLE \`live_room_capacities\` DROP COLUMN \`total_count\``);
    }
    catch { }
    try {
        await prisma.$executeRawUnsafe(`ALTER TABLE \`live_room_capacities\` DROP COLUMN \`live_room_used\``);
    }
    catch { }
    try {
        await prisma.$executeRawUnsafe(`ALTER TABLE \`live_room_capacities\` DROP COLUMN \`office_used\``);
    }
    catch { }
    console.log("\n=== 迁移完成！===");
