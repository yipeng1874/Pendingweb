...
liveRoomCapacityRoutes.get("/live-room-sites", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
...
liveRoomCapacityRoutes.post("/live-room-sites", permissionRequired("task:report:view"), async (req, res) => {
...
liveRoomCapacityRoutes.put("/live-room-sites/:id", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER") {
...
liveRoomCapacityRoutes.delete("/live-room-sites/:id", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER") {
...
liveRoomCapacityRoutes.post("/live-room-capacity/upsert", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER") {
    ...
    const normalized = siteDetails.map((sd) => ({
        ...
        rooms: (sd.rooms ?? []).map((r) => {
            ...
            if (hasAlloc && r.allocations.some((a) => (a.allocated || 0) > 0 || (a.used || 0) > 0)) {
                room.allocations = r.allocations
                    .filter((a) => (a.allocated || 0) > 0 || (a.used || 0) > 0)
                    .map((a) => ({
                    orgId: a.orgId,
                    orgName: a.orgName,
...
liveRoomCapacityRoutes.get("/live-room-capacity/latest", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
