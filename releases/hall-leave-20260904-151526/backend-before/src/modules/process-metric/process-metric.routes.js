...
processMetricRoutes.get("/process-metric/config", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    let baseOrg;
    try {
    ...
    if (config?.teamIds) {
        try {
            teamIds = JSON.parse(config.teamIds);
        }
        catch { /* ignore */ }
    }
...
processMetricRoutes.put("/process-metric/config", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(teamIds))
        return fail(res, "MISSING_PARAMS", "请提供 teamIds 数组", 400);
...
processMetricRoutes.post("/process-metric/upsert", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER") {
    ...
    if (!hallName)
        return fail(res, "MISSING_PARAMS", "请填写厅名", 400);
    if (!recordDate || !/^\d{4}-\d{2}-\d{2}$/.test(String(recordDate)))
        return fail(res, "INVALID_RECORD_DATE", "请提供有效日期（YYYY-MM-DD）", 400);
    if (percentage == null || isNaN(Number(percentage)) || Number(percentage) < 0 || Number(percentage) > 100)
        return fail(res, "INVALID_PERCENTAGE", "百分比应在 0-100 之间", 400);
...
processMetricRoutes.post("/process-metric/upsert-batch", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER")
        return fail(res, "FORBIDDEN", "无权录入数据", 403);
    let baseOrg;
    ...
    if (!Array.isArray(items) || items.length === 0)
        return fail(res, "MISSING_PARAMS", "请至少提交一条数据", 400);
    const uploader = await prisma.user.findUnique({ where: { id: req.userId }, select: { nickname: true } });
    ...
    for (const item of items) {
        try {
            ...
            if (!teamOrgId || !hallName || !recordDate || percentage == null) {
                failCount++;
                continue;
            }
...
processMetricRoutes.get("/process-metric/by-date", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
    ...
    for (const r of allRecords) {
        ...
        if (!teamMap.has(r.teamOrgId))
            teamMap.set(r.teamOrgId, new Map());
        const hallMap = teamMap.get(r.teamOrgId);
        if (!hallMap.has(r.hallName))
            hallMap.set(r.hallName, r);
    }
    ...
    const dateEntries = sortedDates.map((recordDate) => {
        ...
        let teams = Array.from(teamMap.entries()).map(([teamOrgId, hallMap]) => {
            ...
            const halls = Array.from(hallMap.values()).map((h) => ({
                hallName: h.hallName,
...
processMetricRoutes.get("/process-metric/latest-halls", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
    ...
    const teamOrgId = req.query.teamOrgId;
    ...
    for (const r of records) {
        if (!teamMap.has(r.teamOrgId))
            teamMap.set(r.teamOrgId, { teamOrgId: r.teamOrgId, teamOrgName: r.teamOrgName, halls: [] });
...
processMetricRoutes.delete("/process-metric", permissionRequired("task:report:view"), asyncHandler(async (req, res) => {
    const roleCode = req.identity?.roleCode;
    if (roleCode === "TEAM_ADMIN" || roleCode === "HALL_MANAGER") {
