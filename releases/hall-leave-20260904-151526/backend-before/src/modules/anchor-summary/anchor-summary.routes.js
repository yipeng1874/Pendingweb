...
function isWithinDays(date, days, referenceDate) {
...
function matchOperatorByChar(teamName, operatorName) {
...
anchorSummaryRoutes.post("/anchor-summary/upload", permissionRequired("task:report:view"), (req, res, next) => {
    ...
    if (!req.file)
        return fail(res, "NO_FILE", "请选择要上传的 Excel 文件", 400);
    ...
    if (!sheetName)
        return fail(res, "EMPTY_FILE", "Excel 文件为空", 400);
    ...
    for (const row of rows) {
        ...
        if (joinDate) {
            if (isWithinDays(joinDate, 7, refDate)) {
                opStat.within7Days++;
                if (isOnline)
                    opStat.within7DaysOnline++;
                else
                    opStat.within7DaysOffline++;
            }
            if (isWithinDays(joinDate, 20, refDate)) {
                opStat.within20Days++;
                if (isOnline)
                    opStat.within20DaysOnline++;
                else
                    opStat.within20DaysOffline++;
            }
            if (isSameDay(joinDate, refDate))
                opStat.dailyNew++;
        }
    }
    const operatorStats = Array.from(operatorMap.values()).sort((a, b) => b.totalCount - a.totalCount);
    // 获取上传者昵称
...
anchorSummaryRoutes.get("/anchor-summary/latest", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
    ...
    if (record && req.identity?.roleCode === "TEAM_ADMIN" && req.identity?.orgId) {
        ...
        if (teamOrg) {
            ...
            const origOps = record.operatorStats ?? [];
...
anchorSummaryRoutes.get("/anchor-summary/trend", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
    ...
    const probationDays = parseInt(req.query.probationDays, 10) || 0;
    ...
    const points = records.map((r) => {
        ...
        if (r.rawAnchors) {
            ...
            for (const a of anchors) {
                ...
                if (teamOrgName) {
                    ...
                    if (!opName || !matchOperatorByChar(teamOrgName, opName))
                        continue;
                }
                // 试用期内：跳过
            ...
            if (hasOp) {
                for (const a of anchors) {
                    ...
                    // TEAM_ADMIN：过滤非本团队的运营
                    if (teamOrgName && !matchOperatorByChar(teamOrgName, opName))
                        continue;
                    // 试用期内：跳过
                    if (probationDays > 0 && a.joinDate) {
                        const diffMs = refDate.getTime() - new Date(a.joinDate).getTime();
                    ...
                    if (a.joinDate) {
                        ...
                        if (diffDays >= 0 && diffDays < 7) {
                            op.within7Days++;
                            if (a.isOnline)
                                op.within7DaysOnline++;
                            else
                                op.within7DaysOffline++;
                        }
                        if (diffDays >= 0 && diffDays < 20) {
                            op.within20Days++;
                            if (a.isOnline)
                                op.within20DaysOnline++;
                            else
                                op.within20DaysOffline++;
                        }
                    }
                }
                ...
                const orig = r.operatorStats;
                const origTotal = r.totalCount;
                if (orig && origTotal > 0) {
                    const scale = filteredTotal / origTotal;
                    const inferSplit = (total7, totalAll, split) => {
                        if (totalAll <= 0)
                            return 0;
        ...
        return {
            ...
            probationExcluded,
            operatorStats: filteredOperatorStats ?? r.operatorStats,
        };
    });
    // latest 也用过滤后的最新一条
    ...
    return ok(res, {
        ...
        latest: latestRaw
            ? {
                ...
                operatorStats: latest?.operatorStats ?? latestRaw.operatorStats,
