...
async function canAccessTaskItemRecord(taskItemRecordId, userId, identityId) {
    ...
    if (!hasIdentityLink && itemRecord.taskRecord.userId !== userId)
        return null;
    if (hasIdentityLink || itemRecord.taskRecord.userId === userId)
        return itemRecord;
    return null;
...
uploadRoutes.post("/tasks/upload", permissionRequired("task:record:submit"), (req, res, next) => {
    upload.single("file")(req, res, (err) => {
        if (err) {
            if (err.code === "LIMIT_FILE_SIZE")
                return fail(res, "FILE_TOO_LARGE", "图片不得超过1MB", 400);
            if (err.message === "MIME_NOT_ALLOWED")
                return fail(res, "MIME_NOT_ALLOWED", "只支持上传 JPG/PNG/GIF/WebP 格式图片", 400);
    ...
    if (!req.file)
        return fail(res, "NO_FILE", "请选择要上传的图片", 400);
    const taskItemRecordId = req.body?.taskItemRecordId;
    if (!taskItemRecordId)
        return fail(res, "ITEM_RECORD_REQUIRED", "请提供 taskItemRecordId", 400);
...
uploadRoutes.delete("/tasks/attachments/:id", permissionRequired("task:record:submit"), async (req, res) => {
    ...
    if (!attachment)
        return fail(res, "ATTACHMENT_NOT_FOUND", "附件不存在", 404);
    if (attachment.uploadedBy !== req.userId)
        return fail(res, "FORBIDDEN", "无权删除该附件", 403);
    if (attachment.taskItemRecord.taskRecord.exemption?.status === "approved")
        return fail(res, "RECORD_EXEMPTED", "今日任务已豁免，当前不可删除附件", 409);
...
// ── 厅管日常任务专用图片上传 ──────────────────────────────────────────────────
uploadRoutes.post("/tasks/hall-daily/upload", permissionRequired("task:record:submit"), (req, res, next) => {
    upload.single("file")(req, res, (err) => {
        if (err) {
            if (err.code === "LIMIT_FILE_SIZE")
                return fail(res, "FILE_TOO_LARGE", "图片不得超过1MB", 400);
    ...
    if (!hallTaskItemRecordId)
        return fail(res, "ITEM_RECORD_REQUIRED", "请提供 hallTaskItemRecordId", 400);
...
uploadRoutes.delete("/tasks/hall-daily/attachments/:id", permissionRequired("task:record:submit"), async (req, res) => {
