...
async function purgeOldReminders(userId) {
...
function parseImportantFlag(value) {
...
// 获取个人提醒列表
reminderRoutes.get("/tasks/reminders", permissionRequired("task:reminder:manage"), async (req, res) => {
    ...
    const data = await prisma.personalReminder.findMany({
        where: { userId: req.userId, ...(statusParam && REMINDER_STATUS_VALUES.has(statusParam) ? { status: statusParam } : {}) },
        orderBy: [{ status: "asc" }, { isImportant: "desc" }, { remindEnd: "asc" }, { createdAt: "desc" }],
    });
    return ok(res, data);
});
// 创建提醒
reminderRoutes.post("/tasks/reminders", permissionRequired("task:reminder:manage"), async (req, res) => {
    const { title, note, remindAt, remindStart, remindEnd, isImportant } = req.body;
    const normalizedTitle = t(title);
    if (!normalizedTitle)
        return fail(res, "REMINDER_REQUIRED", "提醒标题不能为空", 400);
    const parsedEnd = parseReminderDate(remindEnd ?? remindStart ?? remindAt);
    if (!parsedEnd)
        return fail(res, "REMINDER_END_REQUIRED", "请选择有效的结束时间", 400);
...
// 更新提醒
reminderRoutes.patch("/tasks/reminders/:id", permissionRequired("task:reminder:manage"), async (req, res) => {
    const reminder = await prisma.personalReminder.findFirst({ where: { id: req.params.id, userId: req.userId } });
    if (!reminder)
        return fail(res, "REMINDER_NOT_FOUND", "提醒不存在", 404);
    const data = {};
    ...
    if ("status" in req.body) {
        ...
        if (!REMINDER_STATUS_VALUES.has(status))
            return fail(res, "REMINDER_STATUS_INVALID", "提醒状态无效", 400);
        data.status = status;
    }
...
// 删除提醒
reminderRoutes.delete("/tasks/reminders/:id", permissionRequired("task:reminder:manage"), async (req, res) => {
    const reminder = await prisma.personalReminder.findFirst({ where: { id: req.params.id, userId: req.userId } });
    if (!reminder)
        return fail(res, "REMINDER_NOT_FOUND", "提醒不存在", 404);
...
// 标记完成
reminderRoutes.post("/tasks/reminders/:id/done", permissionRequired("task:reminder:manage"), async (req, res) => {
    const reminder = await prisma.personalReminder.findFirst({ where: { id: req.params.id, userId: req.userId } });
    if (!reminder)
        return fail(res, "REMINDER_NOT_FOUND", "提醒不存在", 404);
