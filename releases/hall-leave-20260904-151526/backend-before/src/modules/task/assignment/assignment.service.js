...
import { assignmentDetailInclude, assignmentListInclude, endOtherActiveDailyAssignments, ensureTemplatePublished, isAssignmentAnchorExcluded, isAssignmentOrgExcluded, nextMidnight, parseDateTime, reconcileDailyAssignments, replaceAssignmentExclusions, replaceAssignmentTargets, } from "./daily-assignment.utils.js";
import { buildTemporaryPreview, buildTemporarySubjectGroups, listTemporaryManagerOrgsWithoutManagers, normalizeTemporaryDraftInput, resolveTemporaryMode, resolveTemporaryRoleCodes, resolveTemporaryTargetUserIds, } from "./temporary-assignment.utils.js";
...
async function duplicateDailyTemplateAsDraft(tx, templateId, createdBy) {
...
async function buildScopeWhere(scopePath, roleCode) {
    if (!scopePath || roleCode === "DEV_ADMIN")
        return {};
    const orgs = await prisma.orgUnit.findMany({
        where: { path: { startsWith: scopePath } },
        select: { id: true },
    });
    return { createdByOrgId: { in: orgs.map((org) => org.id) } };
}
function dedupe(values = []) {
    return Array.from(new Set((values || []).filter(Boolean)));
}
function stripAnchorMigrationSuffix(value) {
    return String(value ?? "").replace(/back-\d{6,8}(?:-\d{9})?$/i, "").trim();
}
function resolveTargetRoleType(mode) {
    if (mode === "ANCHOR")
        return "ANCHOR";
    if (mode === "MANAGER")
        return "ADMIN";
...
async function replaceTargets(tx, assignmentId, orgIds = []) {
...
async function replaceExclusions(tx, assignmentId, excludedOrgIds = [], excludedAnchorProfileIds = []) {
...
async function getTemporaryAssignmentById(tx, id) {
    return tx.taskAssignment.findUnique({
        where: { id },
        include: temporaryAssignmentDetailInclude,
    });
}
function assertTemporaryOwnership(assignment, context) {
    if (context.scopeOrgId && assignment.createdByOrgId !== context.scopeOrgId)
        throw new Error("ASSIGNMENT_NOT_FOUND");
    if (assignment.createdByIdentityId) {
        if (!context.identityId || assignment.createdByIdentityId !== context.identityId)
            throw new Error("TEMP_ASSIGNMENT_OWNER_REQUIRED");
        return;
    }
    if (!context.userId || assignment.createdBy !== context.userId)
        throw new Error("TEMP_ASSIGNMENT_OWNER_REQUIRED");
}
async function createTemporaryAssignment(data) {
...
async function listReusableDailyDrafts(tx, data) {
...
async function cleanupDuplicateDailyDrafts(tx, keepId, draftIds) {
...
async function saveTemporaryDraft(data) {
    return prisma.$transaction(async (tx) => {
        const template = await tx.taskTemplate.findUnique({ where: { id: data.templateId } });
        if (!template)
            throw new Error("TEMPLATE_NOT_FOUND");
        if (template.category !== "TEMPORARY")
        ...
        if (assignmentId) {
            ...
            if (!existing)
                throw new Error("ASSIGNMENT_NOT_FOUND");
            if (existing.category !== "TEMPORARY")
                throw new Error("ASSIGNMENT_CATEGORY_INVALID");
        ...
        const currentScopePath = data.currentScopePath;
        if (currentScopePath && (normalized.orgIds ?? []).length > 0) {
            ...
            const outOfScope = targetOrgs.filter((org) => {
            ...
            if (outOfScope.length > 0)
                throw new Error("ASSIGNMENT_TARGETS_OUT_OF_SCOPE");
        }
...
async function getTemporaryPublishPreview(id, userId, identityId, _scopePath, _roleCode, scopeOrgId) {
    ...
    if (!groups.length)
        throw new Error("TEMP_ASSIGNMENT_AUDIENCE_EMPTY");
...
async function generateTemporaryRecords(tx, assignmentId) {
...
async function publishTemporaryDraft(id, userId, identityId, _scopePath, _roleCode, scopeOrgId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (!assignment.deadlineAt)
            throw new Error("TEMP_ASSIGNMENT_DEADLINE_REQUIRED");
        const now = new Date();
...
async function updateAssignment(id, data, scopePath, roleCode, userId, identityId, scopeOrgId) {
    await reconcileTemporaryAssignments();
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (data.orgIds?.length)
            await replaceAssignmentTargets(tx, id, data.orgIds);
...
async function toggleAssignmentActive(id, isActive, _scopePath, _roleCode, userId, identityId, scopeOrgId) {
    const assignment = await prisma.taskAssignment.findUnique({ where: { id } });
    if (!assignment)
        throw new Error("ASSIGNMENT_NOT_FOUND");
    if (assignment.category === "TEMPORARY") {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const targetOrgIds = targets.map((t) => t.orgId);
...
async function listAssignments(scopePath, roleCode, category, scopeOrgId, userId, identityId, status, limit, offset) {
    await reconcileDailyAssignments(scopePath);
    await reconcileTemporaryAssignments();
    const where = { ...(await buildScopeWhere(scopePath, roleCode)) };
    if (category)
    ...
    if (category === "TEMPORARY") {
        if (identityId)
            where.createdByIdentityId = identityId;
        else if (userId)
    ...
    if (status) {
        ...
        if (!statusList.includes("deleted"))
            where.deletedAt = null;
...
async function getLastDailyExclusionPreset(scopeOrgId, currentScopePath, roleCode) {
    ...
    const previousAnchorIds = Array.from(new Set(anchorExclusions.map((item) => item.anchorProfileId).filter(Boolean)));
    const activeAnchorIdentities = previousAnchorIds.length
        ...
        : [];
    const activeAnchorMap = new Map();
    activeAnchorIdentities.forEach((identity) => {
        if (identity.anchorProfileId && !activeAnchorMap.has(identity.anchorProfileId))
            activeAnchorMap.set(identity.anchorProfileId, identity);
    });
...
async function getAssignmentById(id, _scopePath, _roleCode, scopeOrgId, userId, identityId) {
    ...
    if (!assignment)
        return null;
    if (assignment.category === "TEMPORARY") {
        if (scopeOrgId && assignment.createdByOrgId !== scopeOrgId)
            return null;
        if (identityId && assignment.createdByIdentityId && assignment.createdByIdentityId !== identityId)
            return null;
        if (!assignment.createdByIdentityId && userId && assignment.createdBy !== userId)
            return null;
    }
    return assignment;
}
async function saveDailyDraft(data) {
    return prisma.$transaction(async (tx) => {
        const template = await tx.taskTemplate.findUnique({ where: { id: data.templateId } });
        if (!template)
            throw new Error("TEMPLATE_NOT_FOUND");
        ...
        const matchedDraft = reusableDrafts.find((item) => item.templateId === nextData.templateId);
        let assignmentId = nextData.assignmentId || matchedDraft?.id;
        if (assignmentId) {
            const existing = await tx.taskAssignment.findUnique({ where: { id: assignmentId } });
            if (!existing)
                throw new Error("ASSIGNMENT_NOT_FOUND");
            if (existing.category !== "DAILY")
                throw new Error("ASSIGNMENT_CATEGORY_INVALID");
            if (existing.status !== "draft")
                throw new Error("ASSIGNMENT_NOT_DRAFT");
            await tx.taskAssignment.update({
        ...
        await cleanupDuplicateDailyDrafts(tx, assignmentId, reusableDrafts.map((item) => item.id));
...
async function publishDailyDraft(id, effectMode, _scopePath, _roleCode, scopeOrgId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (nextStatus === "scheduled") {
            ...
            if (scheduledExists)
                throw new Error("DAILY_SCHEDULED_EXISTS");
        }
...
async function createAssignment(data) {
    if (data.category === "TEMPORARY")
        return createTemporaryAssignment(data);
    return prisma.$transaction(async (tx) => {
        const template = await ensureTemplatePublished(tx, data.templateId);
        const now = new Date();
        const orgIds = dedupe(data.orgIds ?? []);
...
async function getTargetUsers(assignmentId) {
    await reconcileDailyAssignments();
    await reconcileTemporaryAssignments();
    const assignment = await prisma.taskAssignment.findUnique({
        where: { id: assignmentId },
    ...
    for (const target of assignment.targets) {
        const identities = await prisma.userIdentity.findMany({
            where: {
                status: "active",
                roleCode: assignment.targetRoleType === "ANCHOR" ? "ANCHOR" : { in: (assignment.targetAdminLevels ?? []) },
...
// ─── 厅管日常任务：草稿保存 ────────────────────────────────────────────────
async function saveHallDailyDraft(data) {
    return prisma.$transaction(async (tx) => {
        const template = await tx.taskTemplate.findUnique({ where: { id: data.templateId } });
        if (!template)
            throw new Error("TEMPLATE_NOT_FOUND");
        if (template.category !== "HALL_DAILY")
            throw new Error("TEMPLATE_CATEGORY_MISMATCH");
        if (template.status === "archived")
            throw new Error("TEMPLATE_ARCHIVED");
        const nextData = {
        ...
        if (assignmentId) {
            ...
            if (!existing)
                throw new Error("ASSIGNMENT_NOT_FOUND");
            if (existing.category !== "HALL_DAILY")
                throw new Error("ASSIGNMENT_CATEGORY_INVALID");
            if (existing.status !== "draft")
                throw new Error("ASSIGNMENT_NOT_DRAFT");
            await tx.taskAssignment.update({
        ...
        // 清理重复草稿
        const duplicateIds = reusableDrafts.filter((item) => item.id !== assignmentId).map((item) => item.id);
        if (duplicateIds.length) {
            await tx.taskAssignmentExclusion.deleteMany({ where: { assignmentId: { in: duplicateIds } } });
            await tx.taskAssignmentTarget.deleteMany({ where: { assignmentId: { in: duplicateIds } } });
            await tx.taskAssignment.deleteMany({ where: { id: { in: duplicateIds } } });
...
// ─── 厅管日常任务：发布预览 ───────────────────────────────────────────────
async function getHallDailyPublishPreview(id, scopeOrgId) {
    const assignment = await prisma.taskAssignment.findUnique({
        where: { id },
        include: {
    ...
    if (!assignment)
        throw new Error("ASSIGNMENT_NOT_FOUND");
    if (assignment.category !== "HALL_DAILY")
        throw new Error("ASSIGNMENT_CATEGORY_INVALID");
    if (scopeOrgId && assignment.createdByOrgId !== scopeOrgId)
        throw new Error("ASSIGNMENT_NOT_FOUND");
    return {
        ...
        targetOrgs: (assignment.targets ?? []).map((t) => ({ id: t.orgId, name: t.org?.name ?? t.orgId })),
        overlappingAssignments: [],
    };
}
// ─── 厅管日常任务：正式发布 ───────────────────────────────────────────────
async function publishHallDailyDraft(id, effectMode, scopeOrgId) {
    return prisma.$transaction(async (tx) => {
        const assignment = await tx.taskAssignment.findUnique({ where: { id }, include: { targets: true } });
        if (!assignment)
            throw new Error("ASSIGNMENT_NOT_FOUND");
...
export const AssignmentService = {
    ...
    getDailyPublishPreview: async (id, _scopePath, _roleCode, scopeOrgId) => {
        const assignment = await prisma.taskAssignment.findUnique({
            where: { id },
            include: {
                template: { select: { id: true, title: true, version: true, category: true, status: true } },
        ...
        return {
            ...
            effectMode: assignment.effectMode ?? "next_midnight",
            targetOrgCount: assignment.targets?.length ?? 0,
            excludedOrgCount: assignment.exclusions?.filter((item) => item.exclusionType === "ORG").length ?? 0,
            excludedAnchorCount: assignment.exclusions?.filter((item) => item.exclusionType === "ANCHOR").length ?? 0,
