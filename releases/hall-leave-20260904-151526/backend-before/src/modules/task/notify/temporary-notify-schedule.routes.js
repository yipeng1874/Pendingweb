 ...
 */
function getTierIndex(daysLeft) {
    if (daysLeft <= 1)
        return 0;
    if (daysLeft <= 3)
        return 1;
...
function getTierDailyCount(schedule, tierIndex) {
...
function getDailyTriggerHours(dailyCount) {
    if (dailyCount <= 0)
        return [];
    if (dailyCount === 1)
        return [13];
...
function getTemporaryNotifySlotKey(date) {
...
async function buildTemporaryNotifyOpenIds(assignmentId) {
    ...
    for (const record of records) {
        ...
        if (group.subjectType === "USER") {
            const uid = record.userId ?? group.subjectUserId;
            if (uid)
                userIdSet.add(uid);
        }
        else {
            record.visibleIdentityLinks.forEach((link) => {
                if (link.userId)
                    userIdSet.add(link.userId);
                if (link.identity?.userId)
                    userIdSet.add(link.identity.userId);
            });
            if (userIdSet.size === 0) {
                group.visibleIdentities?.forEach((identity) => userIdSet.add(identity.userId));
            }
        }
    }
...
async function buildTemporaryReconfirmOpenIds(assignmentId) {
    const assignment = await prisma.taskAssignment.findUnique({
        where: { id: assignmentId },
        include: { targets: true, exclusions: true, template: { select: { id: true, title: true } } },
    });
...
async function processPreDeadlineReconfirmIfNeeded(schedule, assignment, daysLeft, now, processed) {
    ...
    if (!freshAssignment || !freshAssignment.preDeadlineConfirmEnabled)
        return;
    if (freshAssignment.status !== "active" || !freshAssignment.isActive)
        return;
    const confirmSlotKey = `reconfirm-${formatBeijingDate(now)}`;
    // Dedup check
    const existingLog = await prisma.temporaryNotifyTriggerLog?.findUnique({
    ...
    if (result.count === 0) {
        ...
        await prisma.temporaryNotifyTriggerLog?.create({ data: { assignmentId: assignment.id, slotKey: confirmSlotKey } });
        return;
    }
    ...
    try {
        ...
        await prisma.temporaryNotifyTriggerLog?.create({ data: { assignmentId: assignment.id, slotKey: confirmSlotKey } }).catch(() => undefined);
        processed.push({
            identityId: schedule.identityId,
            assignmentId: assignment.id,
...
export async function runTemporaryNotifyScheduleTick(now = new Date()) {
    ...
    // 查所有 enabled 的调度配置
    const schedules = await prisma.temporaryNotifySchedule?.findMany({
        where: { enabled: true },
    });
        ...
        for (const assignment of assignments) {
            ...
            try {
                ...
                if (!openIdRows.length) {
                    ...
                    // 仍写日志，避免重复查
                    await prisma.temporaryNotifyTriggerLog?.create({ data: { assignmentId: assignment.id, slotKey } });
                    continue;
                }
                const delegate = await getFeishuConfigDelegate();
                if (!delegate)
...
temporaryNotifyScheduleRoutes.get("/tasks/notify/temporary-schedule", permissionRequired("task:report:view"), async (req, res) => {
    ...
    if (!identityId)
        return fail(res, "IDENTITY_REQUIRED", "身份未识别", 401);
    const schedule = await prisma.temporaryNotifySchedule?.findUnique({
        where: { identityId },
    });
    return ok(res, {
...
temporaryNotifyScheduleRoutes.put("/tasks/notify/temporary-schedule", permissionRequired("task:report:view"), async (req, res) => {
    if (!canManageTemporaryNotify(req.identity?.roleCode)) {
    ...
    const schedule = await prisma.temporaryNotifySchedule?.upsert({
        where: { identityId },
        create: {
            identityId,
