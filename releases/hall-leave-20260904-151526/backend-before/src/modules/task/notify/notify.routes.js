...
import { formatBeijingDate, getDailyTaskSupplementDeadline, resolveTaskRecordStatus } from "../record/daily-record-time.utils.js";
const DAILY_NOTIFY_INTERVAL_OPTIONS = [
...
const DAILY_NOTIFY_INTERVAL_HOURS_SET = new Set(DAILY_NOTIFY_INTERVAL_OPTIONS.map((item) => item.intervalHours));
const notifyRoutes = Router();
notifyRoutes.use(authRequired, identityRequired);
const t = (v) => (typeof v === "string" ? v.trim() : "");
function canManageDailyNotify(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
function canManageDailyNotifySchedule(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN"].includes(roleCode ?? "");
}
function canManageTemporaryNotify(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
function isValidDailyNotifyIntervalHours(value) {
    return DAILY_NOTIFY_INTERVAL_HOURS_SET.has(value);
}
function getDailyNotifyDefaultPrefix(baseOrgName) {
    return `来自${baseOrgName}提醒`;
}
function getDailyNotifySchedulePayload(baseOrg, schedule) {
    return {
...
function getDailyNotifySlotKey(date) {
...
async function sendFeishuBatchMessage(config, openIds, text) {
...
async function buildDailyNotifyAudience(taskDate, baseOrg, audienceScopePath) {
    ...
    for (const assignment of assignments) {
        const requiredTotalItems = assignment.template?.items?.filter((item) => item.isRequired !== false).length ?? 0;
        const audience = await listAssignmentAudienceMembers(prisma, assignment, normalizedDate);
        for (const member of audience) {
            if (!isOrgPathWithinScope(audienceScopePath, member.hallOrgPath))
                continue;
    ...
    const recordMap = new Map(records.filter((r) => r.subjectKey).map((record) => [`${record.assignmentId}:${record.subjectKey}:${normalizedDate}`, record]));
    ...
    for (const ref of refs) {
        ...
        if (record?.exemption?.status === "approved" || record?.exemption?.status === "pending")
            continue;
        ...
        const requiredDoneItems = record?.itemRecords
            ? record.itemRecords.filter((ir) => ir.status === "done" && ir.taskItem?.isRequired !== false).length
            : 0;
        // 必填项全部完成，视为已完成，跳过通知（与看板判定逻辑完全一致）
        if (totalCount > 0 && requiredDoneItems >= totalCount)
            continue;
        ...
        if (totalCount === 0) {
            ...
            if (dbStatus !== "pending" && dbStatus !== "in_progress")
                continue;
        }
        ...
        if (incompleteCount === 0)
            continue;
        // 确定通知状态标签（用于前端统计展示）
        const status = requiredDoneItems > 0 ? "in_progress" : "pending";
...
function summarizeNotifyAudience(rows) {
    ...
    const configSummaryMap = new Map();
...
export async function executeDailyNotifySend(taskDate, baseOrg, prefix, audienceScopePath) {
    ...
    if (!delegate)
        throw new Error("FEISHU_CONFIG_UNAVAILABLE");
    const configIds = Array.from(new Set(boundRows.map((item) => item.feishuConfigId).filter(Boolean)));
    const configs = await delegate.findMany({ where: { id: { in: configIds }, status: "active" } });
    const configMap = new Map(configs.map((config) => [config.id, config]));
    const bucketMap = new Map();
    for (const row of boundRows) {
        if (!row.feishuConfigId || !row.feishuOpenId)
            continue;
...
export async function runDailyNotifyScheduleTick(now = new Date()) {
    if (getBeijingMinute(now) >= 5) {
        return { checked: false, reason: "OUTSIDE_TOP_OF_HOUR_WINDOW", slotKey: getDailyNotifySlotKey(now), processed: [] };
    }
...
async function resolveTemporaryAssignmentForNotify(assignmentId, scopeOrgId, userId, identityId) {
    ...
    if (assignment.status !== "active" || !assignment.isActive)
        throw new Error("TEMP_ASSIGNMENT_NOT_ACTIVE");
    if (scopeOrgId && assignment.createdByOrgId !== scopeOrgId)
        throw new Error("ASSIGNMENT_NOT_FOUND");
    if (assignment.createdByIdentityId) {
        if (!identityId || assignment.createdByIdentityId !== identityId)
...
async function buildTemporaryNotifyAudience(assignmentId, scopeOrgId, userId, identityId) {
    ...
    for (const record of records) {
        if (record.userId)
            directUserIds.add(record.userId);
        record.visibleIdentityLinks.forEach((link) => {
            if (link.userId)
                visibleUserIds.add(link.userId);
            if (link.identity?.userId)
                visibleUserIds.add(link.identity.userId);
        });
    ...
    for (const record of records) {
        ...
        record.visibleIdentityLinks.forEach((link) => {
            if (link.userId)
                recipientUserIds.add(link.userId);
            if (link.identity?.userId)
                recipientUserIds.add(link.identity.userId);
        });
...
function summarizeTemporaryNotifyAudience(rows) {
    const pendingCount = rows.filter((item) => item.status === "pending").length;
    const inProgressCount = rows.filter((item) => item.status === "in_progress").length;
    const boundRows = rows.filter((item) => item.feishuConfigId && item.feishuOpenId);
    const unboundRows = rows.filter((item) => !item.feishuConfigId || !item.feishuOpenId);
...
notifyRoutes.get("/tasks/notify/daily-feishu/preview", permissionRequired("task:report:view"), async (req, res) => {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后再通知", 400);
...
notifyRoutes.post("/tasks/notify/daily-feishu/send", permissionRequired("task:report:view"), async (req, res) => {
    if (!canManageDailyNotify(req.identity?.roleCode)) {
        return fail(res, "DAILY_NOTIFY_FORBIDDEN", "当前身份无权执行日常任务飞书通知", 403);
    }
...
notifyRoutes.get("/tasks/notify/daily-schedule", permissionRequired("task:report:view"), async (req, res) => {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后再查看配置", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
...
notifyRoutes.put("/tasks/notify/daily-schedule", permissionRequired("task:report:view"), async (req, res) => {
    try {
        if (!canManageDailyNotifySchedule(req.identity?.roleCode)) {
        ...
        if (baseOrg instanceof Error) {
            if (baseOrg.message === "BASE_SCOPE_REQUIRED")
                return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后再保存配置", 400);
            if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
                return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
            if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
                return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
...
notifyRoutes.post("/tasks/notify/daily-schedule/test", permissionRequired("task:report:view"), async (req, res) => {
    if (!canManageDailyNotify(req.identity?.roleCode)) {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后再测试发送", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
...
notifyRoutes.get("/tasks/notify/temporary-feishu/preview", permissionRequired("task:report:view"), async (req, res) => {
    ...
    try {
        ...
        if (error.message === "ASSIGNMENT_CATEGORY_INVALID")
            return fail(res, "ASSIGNMENT_CATEGORY_INVALID", "当前任务不是临时任务", 400);
        if (error.message === "TEMP_ASSIGNMENT_NOT_ACTIVE")
            return fail(res, "TEMP_ASSIGNMENT_NOT_ACTIVE", "仅支持通知进行中的临时任务", 400);
        if (error.message === "TEMP_ASSIGNMENT_OWNER_REQUIRED")
            return fail(res, "TEMP_ASSIGNMENT_OWNER_REQUIRED", "仅任务发起人可发送临时任务通知", 403);
        return fail(res, "TEMP_NOTIFY_PREVIEW_FAILED", error?.message ?? "临时任务通知预览失败", 500);
    }
});
notifyRoutes.post("/tasks/notify/temporary-feishu/send", permissionRequired("task:report:view"), async (req, res) => {
    if (!canManageTemporaryNotify(req.identity?.roleCode)) {
        return fail(res, "TEMP_NOTIFY_FORBIDDEN", "当前身份无权执行临时任务飞书通知", 403);
    }
