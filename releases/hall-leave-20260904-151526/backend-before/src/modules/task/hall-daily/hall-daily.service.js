...
import { formatBeijingDate, makeBeijingDate, getDailyTaskContext, isDailyRecordOverdue, isDailyRecordCollectionClosed, } from "../record/daily-record-time.utils.js";
// ─── 权限常量 ─────────────────────────────────────────────────────────────────
...
// ─── 工具函数 ─────────────────────────────────────────────────────────────────
function ensureHallDailyRole(roleCode) {
    if (!roleCode || !HALL_DAILY_ALLOWED_ROLE_CODES.has(roleCode)) {
        throw new Error("HALL_DAILY_ROLE_FORBIDDEN");
    }
}
function ensureTeamScopeSelected(scopeOrgId) {
    if (!scopeOrgId)
        throw new Error("HALL_DAILY_TEAM_SCOPE_REQUIRED");
}
...
async function resolveTeamOrg(teamOrgId, scopePath, roleCode) {
    const org = await prisma.orgUnit.findFirst({
        where: { id: teamOrgId, status: "active" },
        select: { id: true, path: true, orgType: true, name: true },
    });
    if (!org)
        throw new Error("HALL_DAILY_TEAM_ORG_NOT_FOUND");
    if (org.orgType !== "TEAM")
        throw new Error("HALL_DAILY_TEAM_ORG_REQUIRED");
...
function normalizeHallTaskLinkUrl(linkUrl) {
...
export const HallDailyTemplateService = {
    ...
    async getById(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        ...
        if (!template)
            throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
        if (teamOrgId && template.teamOrgId !== teamOrgId)
            throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
    ...
    async update(id, data, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        return prisma.$transaction(async (tx) => {
            ...
            if (blockedCount > 0)
                throw new Error("HALL_TASK_TEMPLATE_IN_USE");
    ...
    async remove(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        return prisma.$transaction(async (tx) => {
            ...
            if (teamOrgId && template.teamOrgId !== teamOrgId)
                throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
            await resolveTeamOrg(template.teamOrgId, scopePath, roleCode);
            const hasBlocker = template.assignments.some((a) => ["active", "scheduled", "ended"].includes(a.status));
            if (hasBlocker)
                throw new Error("HALL_TASK_TEMPLATE_HAS_ASSIGNMENTS");
            // 只清理 draft 状态的 assignment（active/scheduled/ended 已在上方拦截）
            const cleanableIds = template.assignments.filter((a) => a.status === "draft").map((a) => a.id);
    ...
    async copy(id, operatorUserId, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        ...
        if (!source)
            throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
        if (teamOrgId && source.teamOrgId !== teamOrgId)
            throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
        await resolveTeamOrg(source.teamOrgId, scopePath, roleCode);
        const normalizedItems = normalizeHallTaskItems(source.items.map((item) => ({
            sortOrder: item.sortOrder,
            itemType: item.itemType,
            title: item.title,
            isRequired: item.isRequired,
            linkUrl: item.linkUrl,
            options: item.options?.map((opt) => ({ sortOrder: opt.sortOrder, label: opt.label })) ?? [],
        })));
        return prisma.hallTaskTemplate.create({
            data: {
    ...
    async archive(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
...
export const HallDailyAssignmentService = {
    ...
    async saveDraft(data) {
        ...
        if (!template)
            throw new Error("HALL_TASK_TEMPLATE_NOT_FOUND");
        if (template.teamOrgId !== data.teamOrgId)
            throw new Error("HALL_TASK_TEMPLATE_TEAM_MISMATCH");
        if (template.status === "archived")
            throw new Error("HALL_TASK_TEMPLATE_ARCHIVED");
        // 验证目标厅都属于该团队
        if (data.hallOrgIds.length > 0) {
            ...
            if (invalidHalls.length > 0)
                throw new Error("HALL_DAILY_HALL_NOT_IN_TEAM");
        }
        return prisma.$transaction(async (tx) => {
            ...
            if (assignmentId) {
                ...
                if (!existing)
                    throw new Error("HALL_TASK_ASSIGNMENT_NOT_FOUND");
                if (existing.teamOrgId !== data.teamOrgId)
                    throw new Error("HALL_TASK_ASSIGNMENT_NOT_FOUND");
                if (existing.status !== "draft")
                    throw new Error("HALL_TASK_ASSIGNMENT_NOT_DRAFT");
    ...
    // 发布预览
    async getPublishPreview(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        ...
        if (!assignment)
            throw new Error("HALL_TASK_ASSIGNMENT_NOT_FOUND");
        if (teamOrgId && assignment.teamOrgId !== teamOrgId)
            throw new Error("HALL_TASK_ASSIGNMENT_NOT_FOUND");
        ...
        const targetHallIds = new Set(assignment.targets.map((t) => t.hallOrgId));
        const overlapping = activeAssignments
            .filter((a) => a.targets.some((t) => targetHallIds.has(t.hallOrgId)))
            .map((a) => ({ id: a.id, title: a.template.title, status: a.status }));
        return {
            ...
            targetOrgs: assignment.targets.map((t) => ({ id: t.hallOrg.id, name: t.hallOrg.name })),
            overlappingAssignments: overlapping,
        };
    },
    // 正式发布
    async publish(id, effectMode, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        return prisma.$transaction(async (tx) => {
            ...
            if (assignment.targets.length === 0)
                throw new Error("HALL_TASK_ASSIGNMENT_TARGETS_REQUIRED");
            if (assignment.template.status === "archived")
                throw new Error("HALL_TASK_TEMPLATE_ARCHIVED");
            const now = new Date();
            ...
            if (existingScheduled)
                throw new Error("HALL_TASK_ASSIGNMENT_SCHEDULED_EXISTS");
            // 结束同团队下其他生效中的厅管日常任务
            // - 立即生效：旧任务立即结束
    ...
    async close(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        ...
        if (!["active", "scheduled"].includes(assignment.status))
            throw new Error("HALL_TASK_ASSIGNMENT_CANNOT_CLOSE");
    ...
    async delete(id, scopePath, roleCode, teamOrgId) {
        ensureHallDailyRole(roleCode);
        return prisma.$transaction(async (tx) => {
...
function canReviewHallPath(reviewerIdentity, hallPath) {
    const roleCode = reviewerIdentity?.roleCode;
    if (!roleCode || !["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN"].includes(roleCode))
        return false;
    if (roleCode === "DEV_ADMIN")
        return true;
    const scopePath = reviewerIdentity?.scopePath;
    if (!scopePath || !hallPath)
        return false;
...
export const HallDailyLeaveService = {
    async apply(data) {
        ...
        if (!reason)
            throw new Error("HALL_TASK_LEAVE_REASON_REQUIRED");
        const { record, identity } = await ensureHallManagerOwnsRecord(data.recordId, data.userId);
        if (record.status === "submitted")
            throw new Error("HALL_TASK_LEAVE_NOT_ALLOWED");
        const now = new Date();
        ...
        if (existingPending)
            throw new Error("HALL_TASK_LEAVE_PENDING_EXISTS");
    ...
    async cancel(leaveRequestId, userId) {
        const leave = await prisma.hallTaskLeaveRequest.findUnique({
            where: { id: leaveRequestId },
            include: { taskRecord: true },
        });
        if (!leave || leave.applicantUserId !== userId)
            throw new Error("HALL_TASK_LEAVE_REQUEST_NOT_FOUND");
        if (leave.status !== "pending")
            throw new Error("HALL_TASK_LEAVE_NOT_PENDING");
        if (isDailyRecordCollectionClosed(leave.taskRecord.recordDate, new Date()))
            throw new Error("HALL_TASK_SUPPLEMENT_DEADLINE_PASSED");
    ...
    async review(data) {
        if (data.action === "rejected" && !data.comment?.trim())
            throw new Error("HALL_TASK_LEAVE_COMMENT_REQUIRED");
        const leave = await prisma.hallTaskLeaveRequest.findUnique({
...
export const HallDailyRecordService = {
    ...
    async getMyRecords(userId) {
        ...
        const hallOrgIds = hallManagerIdentities.map((i) => i.orgId).filter(Boolean);
        ...
        for (const record of records) {
            if (record.status !== "submitted" && isDailyRecordOverdue(record.recordDate, now)) {
                ...
                updated.push({ ...record, status: "overdue" });
            }
            else {
                updated.push(record);
            }
    ...
    async submitItemRecord(data) {
        ...
        if (!allowed)
            throw new Error("HALL_TASK_RECORD_NOT_FOUND");
        if (record.status === "submitted")
            throw new Error("HALL_TASK_RECORD_ALREADY_SUBMITTED");
        ...
        const itemRecord = await prisma.hallTaskItemRecord.upsert({
            ...
            create: {
                ...
                answerOptions: data.answerOptions ? data.answerOptions : undefined,
            ...
            update: {
                ...
                answerOptions: data.answerOptions !== undefined ? data.answerOptions : undefined,
     ...
     * 整条 Record 提交
     */
    async submitRecord(recordId, userId) {
        const record = await prisma.hallTaskRecord.findUnique({
            where: { id: recordId },
            include: {
        ...
        const doneItemIds = new Set(record.itemRecords.filter((ir) => ir.status === "done").map((ir) => ir.taskItemId));
        const incomplete = requiredItems.filter((item) => !doneItemIds.has(item.id));
        if (incomplete.length > 0)
            throw new Error("HALL_TASK_RECORD_INCOMPLETE");
