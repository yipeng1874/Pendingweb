...
function parseContributionLines(rawText) {
    if (!rawText?.trim())
        return [];
    return rawText
        .split("\n")
...
function isRecordValid(record) {
    if (record.subjectType === "ORG")
        return true;
    return !record.user || record.user.status === "active";
}
function getEffectiveRecordStatus(record, now = new Date()) {
...
function getLatestHallDailyLeaveStatus(record) {
...
function resolveHallDailyReportStatus(record, taskDate, now = new Date()) {
    if (!record)
        return null;
    ...
    if (isDailyRecordOverdue(record.recordDate ?? taskDate, now))
        return "overdue";
    return record.doneItems > 0 ? "in_progress" : "pending";
}
function serializeHallDailyLeaveRequest(record) {
...
function canViewDailyDashboard(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
function canViewTemporaryDashboard(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
function canManageTemporaryNotify(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
function normalizeTemporaryDashboardFilter(value) {
    if (value === "submitted" || value === "in_progress" || value === "pending" || value === "overdue")
        return value;
    return "all";
}
function matchesTemporaryDashboardKeyword(record, keyword) {
...
function buildTemporaryDashboardRecord(record, anchorProfileMap, now = new Date()) {
    ...
    return {
        ...
        submissionCount: Array.isArray(record.itemRecords) ? new Set(record.itemRecords.map((item) => item.taskItemId)).size : 0,
        visibleIdentityNames: visibleIdentityLinks.map((link) => link.userName ?? link.user?.nickname ?? link.identityId).filter(Boolean),
        exemptionStatus: record.exemption?.status ?? null,
        exemptionReason: record.exemption?.reason ?? null,
    };
}
function summarizeTemporaryRecords(records) {
...
function summarizeTemporaryOrgNodes(orgs, records, childOrgIds) {
...
async function loadAnchorProfileSnapshotMapByUserIds(userIds) {
    if (!userIds.length)
        return new Map();
    ...
    return new Map(anchorProfiles.filter((p) => p.boundUserId != null).map((profile) => [profile.boundUserId, profile]));
}
async function loadIdentityLabelMap(identityIds) {
...
async function enrichTemporaryDashboardRecords(records) {
    const creatorIdentityIds = records.map((record) => record.assignment?.createdByIdentityId).filter((value) => Boolean(value));
    const submitterIdentityIds = records.map((record) => record.lastSubmittedByIdentityId).filter((value) => Boolean(value));
...
async function listTemporaryDashboardRecordsByWhere(where) {
...
function resolveDailyDashboardStatus(record) {
    ...
    if (completed)
        return supplemented ? "supplemented" : "completed";
    if (record.requiredDoneItems > 0)
        return "in_progress";
    return "pending";
}
function calculateCompletionRate(completed, total, approvedExemptions) {
    const assessedTotal = Math.max(0, total - approvedExemptions);
    return assessedTotal > 0 ? Math.round((completed / assessedTotal) * 100) : 0;
}
async function loadDailyDashboardAudience(taskDate, baseOrg, viewerScopePath, reconcile = true) {
    if (reconcile)
    ...
    const assignmentRequiredItemCountMap = new Map();
    // taskItemId -> { title, isRequired }
    const taskItemMetaMap = new Map();
    for (const assignment of assignments) {
        assignmentRequiredItemCountMap.set(assignment.id, assignment.template?.items?.filter((item) => item.isRequired !== false).length ?? 0);
        ...
        const audience = await listAssignmentAudienceMembers(prisma, assignment, taskDate);
        for (const member of audience) {
            if (!member.hallOrgId || !member.hallOrgName || !member.hallOrgPath)
                continue;
            if (!(member.hallOrgPath === viewerScopePath || member.hallOrgPath.startsWith(`${viewerScopePath}/`) || viewerScopePath.startsWith(`${member.hallOrgPath}/`)))
                continue;
            const uniqueSubjectKey = `${member.subjectKey}:${taskDate}`;
            if (resolvedSubjectKeys.has(uniqueSubjectKey))
                continue;
    ...
    const reviewerUserIds = Array.from(new Set(records.map((record) => record.exemption?.reviewedBy).filter((value) => Boolean(value))));
    ...
    const recordMap = new Map(records.map((record) => {
        ...
        const completedAt = doneRequiredItems.length
            ? doneRequiredItems.reduce((latest, item) => {
                if (!item.doneAt)
                    return latest;
                if (!latest)
                    return item.doneAt;
        ...
        const doneItemIdSet = new Set(record.itemRecords.filter((ir) => ir.status === "done").map((ir) => ir.taskItemId));
...
reportRoutes.get("/tasks/report/assignments/:id/progress", permissionRequired("task:report:view"), async (req, res) => {
    const assignmentId = req.params.id;
    const assignment = await AssignmentService.getById(assignmentId, req.identity?.scopePath, req.identity?.roleCode, undefined, req.userId, req.identity?.id);
    if (!assignment)
        return fail(res, "ASSIGNMENT_NOT_FOUND", "发放任务不存在", 404);
    ...
    const subjectUserIds = Array.from(new Set(allRecords.map((record) => record.subjectUserId).filter(Boolean)));
...
reportRoutes.get("/tasks/report/assignments/:id/daily", permissionRequired("task:report:view"), async (req, res) => {
    const assignmentId = req.params.id;
    ...
    for (const record of records) {
        ...
        if (!byDate[date])
            byDate[date] = { date, total: 0, submitted: 0, overdue: 0, exempted: 0 };
        ...
        if (effectiveStatus === "submitted")
            byDate[date].submitted++;
        if (effectiveStatus === "overdue")
            byDate[date].overdue++;
    }
    return ok(res, Object.values(byDate).sort((left, right) => right.date.localeCompare(left.date)));
});
reportRoutes.get("/tasks/report/daily-dashboard", permissionRequired("task:report:view"), async (req, res) => {
    ...
    const baseOrg = await resolveBaseScopeOrg(scopeOrgId, req.identity).catch((error) => error);
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先切换并选择基地后查看日常任务看板", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
    ...
    for (const ref of hallMemberRefs) {
        ...
        if (!hall)
            continue;
        const chain = [hall, hall.parentOrgId ? nodes.get(hall.parentOrgId) : undefined, nodes.get(baseOrg.id)].filter(Boolean);
    ...
    const itemDoneCountMap = new Map();
    // itemId -> teamOrgId -> { teamName, done, inProgress, pending, total }
    const itemTeamMap = new Map();
    ...
    for (const ref of hallMemberRefs) {
        ...
        for (const [itemId, meta] of taskItemMetaMap.entries()) {
            ...
            // 按团队统计
            if (!itemTeamMap.has(itemId))
                itemTeamMap.set(itemId, new Map());
...
reportRoutes.get("/tasks/report/daily-dashboard/export-month", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewDailyDashboard(req.identity?.roleCode)) {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后再导出", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权导出该基地", 403);
    ...
    const dates = [];
    for (let date = startDate; date <= endDate; date = addBeijingDays(date, 1))
        dates.push(date);
    try {
...
reportRoutes.get("/tasks/report/daily-dashboard/teams/:teamOrgId/children", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewDailyDashboard(req.identity?.roleCode)) {
        return fail(res, "DAILY_DASHBOARD_FORBIDDEN", "当前身份无权查看日常任务看板", 403);
    }
    const taskDate = t(req.query.taskDate) || formatBeijingDate(new Date());
    ...
    if (!teamRefs.length)
        return fail(res, "TEAM_NOT_FOUND", "团队不存在或当前范围下无数据", 404);
...
reportRoutes.get("/tasks/report/daily-range-stats", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewDailyDashboard(req.identity?.roleCode)) {
        return fail(res, "DAILY_DASHBOARD_FORBIDDEN", "当前身份无权查看日常任务看板", 403);
    }
    const startDate = t(req.query.startDate);
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先切换并选择基地后查看", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
    ...
    const resolvedSubjectKeys = new Set();
    for (const assignment of assignments) {
        const audience = await listAssignmentAudienceMembers(prisma, assignment, endDate);
        for (const member of audience) {
            if (!member.hallOrgId || !member.hallOrgName || !member.hallOrgPath)
                continue;
            if (!(member.hallOrgPath === viewerScopePath || member.hallOrgPath.startsWith(`${viewerScopePath}/`) || viewerScopePath.startsWith(`${member.hallOrgPath}/`)))
                continue;
            if (resolvedSubjectKeys.has(member.subjectKey))
                continue;
    ...
    const teamAudienceCount = new Map();
    const teamOrgNameMap = new Map();
    for (const m of audienceMembers) {
        ...
        if (!teamOrgNameMap.has(teamOrgId))
            teamOrgNameMap.set(teamOrgId, teamOrgName);
    }
...
// ── 厅管历史完成率：多日期范围统计 ─────────────────────────────────────────
reportRoutes.get("/tasks/report/hall-daily-range-stats", permissionRequired("task:report:view"), async (req, res) => {
    ...
    function enumerateHallDates(start, end) {
        const result = [];
        const cur = new Date(`${start}T00:00:00Z`);
        const endD = new Date(`${end}T00:00:00Z`);
        let guard = 0;
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后查看", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
    ...
    const allHallTargetIds = new Set();
    for (const assignment of assignments) {
    ...
    const hallTeamMap = new Map();
    const scopeFilteredHallIds = new Set();
...
reportRoutes.get("/tasks/report/daily-dashboard/halls/:hallOrgId/details", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewDailyDashboard(req.identity?.roleCode)) {
        return fail(res, "DAILY_DASHBOARD_FORBIDDEN", "当前身份无权查看日常任务看板", 403);
    }
    const taskDate = t(req.query.taskDate) || formatBeijingDate(new Date());
    ...
    if (!hall)
        return fail(res, "HALL_NOT_FOUND", "厅不存在或已停用", 404);
    if (!(hall.path === viewerScopePath || hall.path.startsWith(`${viewerScopePath}/`) || viewerScopePath.startsWith(`${hall.path}/`))) {
    ...
    const details = refs
        ...
        .sort((left, right) => {
        const order = { pending: 0, in_progress: 1, supplemented: 2, completed: 3 };
...
reportRoutes.get("/tasks/report/daily-dashboard/halls/:hallOrgId/anchors/:userId/items", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewDailyDashboard(req.identity?.roleCode)) {
        return fail(res, "DAILY_DASHBOARD_FORBIDDEN", "当前身份无权查看日常任务看板", 403);
    }
    const taskDate = t(req.query.taskDate) || formatBeijingDate(new Date());
    ...
    if (!ref)
        return fail(res, "ANCHOR_NOT_FOUND", "当前厅下未找到该主播任务记录", 404);
    ...
    if (!assignment?.template)
        return fail(res, "ASSIGNMENT_TEMPLATE_NOT_FOUND", "任务模板不存在", 404);
...
reportRoutes.get("/tasks/report/temporary-dashboard/active-mode-counts", permissionRequired("task:report:view"), async (req, res) => {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先切换并选择基地后查看临时任务看板", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
    ...
    const identityFilter = req.identity?.id
        ...
        : req.userId
            ...
            : {};
    const modes = ["ACCOUNT", "ANCHOR", "MANAGER"];
    const counts = await Promise.all(modes.map((mode) => prisma.taskAssignment.count({
        where: {
            category: "TEMPORARY",
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const scopeOrgId = t(req.query.scopeOrgId) || undefined;
    ...
    const statusWhere = lifecycle === "ended" ? { in: ["ended", "deleted"] } : { in: ["active"] };
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments/:id/summary", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const assignmentId = t(req.params.id);
    const assignment = await AssignmentService.getById(assignmentId, req.identity?.scopePath, req.identity?.roleCode, undefined, req.userId, req.identity?.id);
    if (!assignment || assignment.category !== "TEMPORARY")
        return fail(res, "ASSIGNMENT_NOT_FOUND", "临时任务不存在", 404);
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments/:id/records", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const assignmentId = t(req.params.id);
    ...
    const anchorProfiles = subjectUserIds.length
        ...
        : [];
    const anchorProfileMap = new Map(anchorProfiles.filter((p) => p.boundUserId != null).map((profile) => [profile.boundUserId, profile]));
    const now = new Date();
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments/:id/anchor-team-nodes", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const assignmentId = t(req.params.id);
    ...
    if (!assignment || assignment.category !== "TEMPORARY")
        return fail(res, "ASSIGNMENT_NOT_FOUND", "临时任务不存在", 404);
    if (assignment.temporaryMode !== "ANCHOR")
        return fail(res, "TEMPORARY_DASHBOARD_MODE_INVALID", "仅主播式任务支持团队-厅钻取", 400);
    const rawRecords = await listTemporaryDashboardRecordsByWhere({ assignmentId, subjectOrgId: { not: null } });
    const hallIds = Array.from(new Set(rawRecords.map((record) => record.subjectOrgId).filter(Boolean)));
    const halls = hallIds.length
        ...
        : [];
    const subjectUserIds = Array.from(new Set(rawRecords.map((record) => record.subjectUserId).filter(Boolean)));
    ...
    const teamIds = Array.from(new Set(halls.map((org) => org.parentId).filter(Boolean)));
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments/:id/anchor-teams/:teamOrgId/hall-nodes", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const assignmentId = t(req.params.id);
...
reportRoutes.get("/tasks/report/temporary-dashboard/assignments/:id/anchor-halls/:hallOrgId/records", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    const assignmentId = t(req.params.id);
    ...
    if (!assignment || assignment.category !== "TEMPORARY")
        return fail(res, "ASSIGNMENT_NOT_FOUND", "临时任务不存在", 404);
    if (assignment.temporaryMode !== "ANCHOR")
        return fail(res, "TEMPORARY_DASHBOARD_MODE_INVALID", "仅主播式任务支持组织树钻取", 400);
    const allRecords = await prisma.taskRecord.findMany({
...
reportRoutes.get("/tasks/report/temporary-dashboard/records/:recordId/detail", permissionRequired("task:report:view"), async (req, res) => {
    if (!canViewTemporaryDashboard(req.identity?.roleCode)) {
        return fail(res, "TEMPORARY_DASHBOARD_FORBIDDEN", "当前身份无权查看临时任务看板", 403);
    }
    ...
    if (!record || record.assignment.category !== "TEMPORARY" || !enrichedRecord)
        return fail(res, "RECORD_NOT_FOUND", "主体记录不存在", 404);
    const assignment = await AssignmentService.getById(record.assignmentId, req.identity?.scopePath, req.identity?.roleCode, undefined, req.userId, req.identity?.id);
    if (!assignment)
        return fail(res, "RECORD_FORBIDDEN", "当前身份无权查看该主体记录", 403);
    ...
    const contributionIdentityIds = record.assignment.temporaryMode === "MANAGER"
        ? Array.from(new Set([
            ...
            ...(record.itemRecords ?? []).map((item) => item.completedByIdentityId).filter(Boolean),
        ]))
        : [];
    ...
    return ok(res, {
        ...
        items: record.assignment.template?.items.map((item) => {
            const itemRecord = itemRecordMap.get(item.id);
            const contributionSummary = itemContributionSummaries.find((entry) => entry.taskItemId === item.id);
            return {
                taskItemId: item.id,
                title: item.title,
                itemType: item.itemType,
...
// ── 厅管日常任务看板 ──────────────────────────────────────────────────────────
reportRoutes.get("/tasks/report/hall-daily-dashboard", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    ...
    const taskDate = t(req.query.taskDate) || formatBeijingDate(new Date());
    const hallOrgId = req.identity?.orgId;
    if (!hallOrgId) {
    ...
    if (!hall)
        return fail(res, "HALL_NOT_FOUND", "当前直播厅不存在或已停用", 404);
    const now = new Date();
    const today = formatBeijingDate(now);
...
reportRoutes.get("/tasks/report/hall-daily-dashboard/overview", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    const allowedAdminRoles = ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN"];
    if (!allowedAdminRoles.includes(roleCode)) {
    ...
    if (baseOrg instanceof Error) {
        if (baseOrg.message === "BASE_SCOPE_REQUIRED")
            return fail(res, "BASE_SCOPE_REQUIRED", "请先选择基地后查看厅管日常任务看板", 400);
        if (baseOrg.message === "SCOPE_ORG_NOT_FOUND")
            return fail(res, "SCOPE_ORG_NOT_FOUND", "当前基地不存在或已停用", 404);
        if (baseOrg.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权查看该基地", 403);
    ...
    const teamTemplateTitleMap = new Map();
...
reportRoutes.get("/tasks/report/hall-daily-dashboard/teams/:teamOrgId/halls", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    const allowedAdminRoles = ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN"];
    if (!allowedAdminRoles.includes(roleCode)) {
    ...
    if (!team)
        return fail(res, "TEAM_ORG_NOT_FOUND", "团队不存在或已停用", 404);
...
reportRoutes.get("/tasks/report/hall-daily-dashboard/halls/:hallOrgId/detail", permissionRequired("task:report:view"), async (req, res) => {
    const roleCode = req.identity?.roleCode;
    const allowedAdminRoles = ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN"];
    if (!allowedAdminRoles.includes(roleCode)) {
...
reportRoutes.get("/tasks/report/summary", permissionRequired("task:report:view"), async (req, res) => {
    ...
    const orgFilter = scopePath && roleCode !== "DEV_ADMIN"
        ...
        : {};
    const templateOrgWhere = Array.isArray(orgFilter.createdByOrgId?.in)
        ? { orgId: { in: orgFilter.createdByOrgId.in } }
        : {};
