...
const workflowUpload = multer({
    ...
    fileFilter(_req, file, cb) {
        if (WORKFLOW_ALLOWED_MIME.includes(file.mimetype))
            cb(null, true);
        else
...
function canManageWorkflow(roleCode) {
    return ["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(roleCode ?? "");
}
async function loadOperatorContext(identity) {
...
workflowTaskRoutes.get("/tasks/collaboration/workflows/bootstrap", identityRequired, async (req, res) => {
    ...
    const categoryPriority = ["subordinate_anchor", "subordinate_manager", "peer_manager"];
    const assignees = new Map();
    function classifyIdentity(item) {
        if (identity.roleCode === "DEV_ADMIN")
            return item.roleCode === "ANCHOR" ? "subordinate_anchor" : "subordinate_manager";
        ...
        if (item.roleCode === "ANCHOR")
            return isSameLevel || isSubordinate ? "subordinate_anchor" : null;
        if (isSubordinate)
            return "subordinate_manager";
        if (isSameLevel)
            return "peer_manager";
        return null;
    }
    for (const item of identities) {
        ...
        if (existing) {
            if (!existing.roleCodes.includes(item.roleCode))
                existing.roleCodes.push(item.roleCode);
            ...
            if (!existing.anchorDouyinNo && item.anchorProfile?.douyinNo)
                existing.anchorDouyinNo = item.anchorProfile.douyinNo;
            if (!existing.anchorNickname && item.anchorProfile?.nickname)
                existing.anchorNickname = item.anchorProfile.nickname;
            continue;
        }
...
workflowTaskRoutes.get("/tasks/collaboration/workflows/assignees/search", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity || !canManageWorkflow(identity.roleCode)) {
...
workflowTaskRoutes.post("/tasks/collaboration/workflows", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity || !canManageWorkflow(identity.roleCode)) {
    ...
    const stepInputs = Array.isArray(req.body?.steps) ? req.body.steps : [];
    if (!title)
        return fail(res, "WORKFLOW_TITLE_REQUIRED", "请填写任务标题", 400);
    if (stepInputs.length < 1)
        return fail(res, "WORKFLOW_STEPS_REQUIRED", "请至少配置一个流转节点", 400);
    const { operatorOrg, operatorUser, scopePath } = await loadOperatorContext(identity);
    if (!scopePath || !operatorOrg)
        return fail(res, "WORKFLOW_SCOPE_REQUIRED", "当前身份缺少组织范围，无法发布协同任务", 400);
    const targetOrg = operatorOrg;
    const uniqueAssigneeIds = Array.from(new Set(stepInputs.map((item) => item.assigneeUserId?.trim()).filter(Boolean)));
    ...
    const assigneeIdentities = uniqueAssigneeIds.length
        ...
        : [];
    const assigneeContextMap = new Map();
    for (const row of assigneeIdentities) {
        if (assigneeContextMap.has(row.userId))
            continue;
...
workflowTaskRoutes.get("/tasks/collaboration/workflows/mine", async (req, res) => {
...
workflowTaskRoutes.post("/tasks/collaboration/workflows/:taskId/steps/:stepId/submit", async (req, res) => {
    const userId = req.userId;
    if (!userId)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    ...
    if (!task)
        return fail(res, "TASK_NOT_FOUND", "任务不存在或无权访问", 404);
...
workflowTaskRoutes.get("/tasks/collaboration/workflows/issued", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity || !canManageWorkflow(identity.roleCode)) {
...
workflowTaskRoutes.get("/tasks/collaboration/workflows/:taskId", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity || !canManageWorkflow(identity.roleCode)) {
    ...
    if (!task)
        return fail(res, "TASK_NOT_FOUND", "任务不存在", 404);
...
workflowTaskRoutes.post("/tasks/collaboration/workflows/:taskId/steps/:stepId/answer", async (req, res) => {
    const userId = req.userId;
    if (!userId)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    const { taskId, stepId } = req.params;
    ...
    const answer = {
        ...
        answerOptions: Array.isArray(raw.answerOptions) ? raw.answerOptions : undefined,
        isLinkConfirmed: typeof raw.isLinkConfirmed === "boolean" ? raw.isLinkConfirmed : undefined,
        attachmentUrls: Array.isArray(raw.attachmentUrls) ? raw.attachmentUrls : undefined,
    };
    if (!answer.questionId)
        return fail(res, "QUESTION_ID_REQUIRED", "缺少 questionId", 400);
...
workflowTaskRoutes.post("/tasks/collaboration/workflows/attachments/upload", authRequired, (req, res, next) => {
    workflowUpload.single("file")(req, res, (err) => {
        if (err) {
            if (err.code === "LIMIT_FILE_SIZE")
                return fail(res, "FILE_TOO_LARGE", "文件不得超过 20MB", 400);
            if (err.message === "MIME_NOT_ALLOWED")
                return fail(res, "MIME_NOT_ALLOWED", "不支持该文件类型", 400);
    ...
    if (!req.file)
        return fail(res, "NO_FILE", "请选择要上传的文件", 400);
