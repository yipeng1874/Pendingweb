...
export async function listWorkflowTasks() {
    ...
    return tasks.map((t) => mapDbTaskToRecord(t));
}
export async function getWorkflowTasksForUser(userId) {
    const now = new Date();
    const tasks = await prisma.workflowTask.findMany({
        where: {
            status: { not: "ended" },
...
export async function getWorkflowTasksByIssuer(userId) {
...
export async function getWorkflowTaskById(taskId) {
    ...
    if (!t)
        return null;
    return mapDbTaskToRecord(t);
}
// ─── 提交节点（一次性全量提交，兼容旧接口） ────────────────────────────────────
export async function submitWorkflowStep(taskId, stepId, userId, answers) {
    ...
    if (step.assigneeUserId !== userId)
        return { success: false, error: "FORBIDDEN" };
    if (step.status === "completed")
        return { success: false, error: "STEP_ALREADY_COMPLETED" };
    const now = new Date();
 ...
 */
export async function saveStepQuestionAnswer(taskId, stepId, userId, answer) {
    const step = await prisma.workflowStep.findUnique({
        where: { id: stepId },
    ...
    const allRequiredFilled = step.questions.every((q) => {
        ...
        if (!ans)
            return false;
        if (q.itemType === "FILL_BLANK" || q.itemType === "QA")
