...
async function resolveBaseOrg(hallPath) {
...
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.get("/tasks/collaboration/broadcast/bootstrap", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    // 高权限账号：提示移步临时任务
    if (["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN"].includes(identity.roleCode)) {
        return ok(res, {
            allowed: false,
    ...
    for (const row of anchorIdentities) {
        if (anchorMap.has(row.userId)) {
            ...
            if (!existing.douyinNo && row.anchorProfile?.douyinNo)
                existing.douyinNo = row.anchorProfile.douyinNo;
            if (!existing.douyinUid && row.anchorProfile?.douyinUid)
                existing.douyinUid = row.anchorProfile.douyinUid;
            if (!existing.anchorNickname && row.anchorProfile?.nickname)
                existing.anchorNickname = row.anchorProfile.nickname;
            continue;
        }
    ...
    const anchors = Array.from(anchorMap.values()).sort((a, b) => a.nickname.localeCompare(b.nickname));
    ...
    for (const row of hallManagerIdentities) {
        if (!row.orgId || !row.org || hallManagerMap.has(row.userId))
            continue;
    ...
    const hallManagers = Array.from(hallManagerMap.values()).sort((a, b) => a.orgName.localeCompare(b.orgName) || a.nickname.localeCompare(b.nickname));
    return ok(res, {
        allowed: true,
        redirectHint: null,
        operator: {
...
// 创建群发主播任务
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.post("/tasks/collaboration/broadcast", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    if (identity.roleCode !== "HALL_MANAGER") {
    ...
    const hallOrg = identity.orgId
        ...
        : null;
    if (!hallOrg)
        return fail(res, "HALL_NOT_FOUND", "当前身份未关联有效厅组织", 400);
    const baseOrg = await resolveBaseOrg(hallOrg.path);
    if (!baseOrg)
        return fail(res, "BASE_NOT_FOUND", "当前直播厅未关联有效基地", 400);
    ...
    if (!title)
        return fail(res, "TITLE_REQUIRED", "请填写任务标题", 400);
    const description = typeof req.body?.description === "string" ? req.body.description.trim() : undefined;
    const dueAt = typeof req.body?.dueAt === "string" && req.body.dueAt.trim()
        ? req.body.dueAt.trim()
        : null;
    const recipientType = req.body?.recipientType === "HALL_MANAGER" ? "HALL_MANAGER" : "ANCHOR";
    ...
    const selectedRecipientUserIds = Array.isArray(rawRecipientIds)
        ? Array.from(new Set(rawRecipientIds
            .filter((v) => typeof v === "string" && Boolean(v.trim()))
            .map((v) => v.trim())))
        : [];
    if (selectedRecipientUserIds.length === 0) {
        return fail(res, "RECIPIENT_REQUIRED", recipientType === "ANCHOR" ? "请至少选择一位主播" : "请至少选择一位其他厅管", 400);
    }
    ...
    for (const row of validRecipientIdentities) {
        if (validRecipientMap.has(row.userId))
            continue;
    ...
    if (invalidIds.length > 0) {
        return fail(res, "RECIPIENT_SCOPE_INVALID", recipientType === "ANCHOR"
            ? "存在不属于本厅的主播，请刷新后重试"
            : "存在不属于当前基地的厅管，请刷新后重试", 400);
    }
    // ── 组装题目 ──────────────────────────────────────────────────────────────
    const questions = rawQuestions.map((q) => ({
        ...
        options: q.itemType === "SINGLE_CHOICE" || q.itemType === "MULTI_CHOICE"
            ? q.options.map((o) => String(o).trim()).filter(Boolean)
            : [],
        linkUrl: q.itemType === "LINK" && typeof q.linkUrl === "string" && q.linkUrl.trim()
            ? q.linkUrl.trim()
            : null,
    }));
    // ── 创建任务 ──────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.get("/tasks/collaboration/broadcast/mine", async (req, res) => {
...
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.post("/tasks/collaboration/broadcast/:taskId/answer", async (req, res) => {
    const userId = req.userId;
    if (!userId)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
...
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.get("/tasks/collaboration/broadcast/issued", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    if (identity.roleCode !== "HALL_MANAGER") {
...
// ─────────────────────────────────────────────────────────────────────────────
broadcastTaskRoutes.get("/tasks/collaboration/broadcast/issued/:taskId/anchor-answers", identityRequired, async (req, res) => {
    const identity = req.identity;
    if (!identity)
        return fail(res, "UNAUTHORIZED", "请先登录", 401);
    if (identity.roleCode !== "HALL_MANAGER") {
    ...
    if (!anchorRecords)
        return fail(res, "TASK_NOT_FOUND", "任务不存在或无权限", 404);
