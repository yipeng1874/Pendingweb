...
import { assignmentDetailInclude, isAssignmentAnchorExcluded, isAssignmentOrgExcluded, listAssignmentAudienceMembers, reconcileDailyAssignments, } from "../assignment/daily-assignment.utils.js";
import { formatBeijingDate, getDailyTaskContext, getDailyTaskDayEnd, getDailyTaskSupplementDeadline, isDailyRecordCollectionClosed, resolveTaskRecordStatus, } from "./daily-record-time.utils.js";
...
async function ensureVisibleRecord(recordId, userId, identityId, include) {
...
function canReviewDailyExemption(identity, record) {
    if (!identity.roleCode || !["DEV_ADMIN", "HQ_ADMIN", "BASE_ADMIN", "TEAM_ADMIN", "HALL_MANAGER"].includes(identity.roleCode))
        return false;
    if (identity.roleCode === "DEV_ADMIN")
        return true;
    const targetScopePaths = (record.user?.identities ?? []).map((item) => item.scopePath).filter((value) => Boolean(value));
    if (!targetScopePaths.length || !identity.scopePath)
        return false;
    return targetScopePaths.some((scopePath) => scopePath === identity.scopePath || scopePath.startsWith(`${identity.scopePath}/`));
}
async function ensureIdentityLink(taskRecordId, identity) {
...
function canSupplementSubmitted(record) {
...
async function enrichRecordDisplay(records) {
    ...
    const itemCompletedIdentityIds = records
        .flatMap((record) => (record.itemRecords ?? []).map((item) => item.completedByIdentityId))
        .filter((value) => Boolean(value));
    ...
    return records.map((record) => {
        ...
        return {
            ...
            visibleIdentityLinks: (record.visibleIdentityLinks ?? []).map((link) => ({
                ...link,
            ...
            itemRecords: (record.itemRecords ?? []).map((item) => ({
                ...item,
...
function buildDailySubjectKey(userId) {
    return `USER:${userId}`;
}
function applyEffectiveStatus(record, now = new Date()) {
    return {
        ...record,
...
function isAssignmentEffectiveAt(assignment, at) {
...
function matchesDailyAssignment(assignment, identity) {
    const isInScope = assignment.targets.some((target) => {
    ...
    const adminLevels = assignment.targetAdminLevels ?? [];
...
function pickDailyAssignment(assignments, identity, at) {
...
async function resolveDirectExemptionTargets(input, identity) {
    if (!identity.roleCode || !DAILY_EXEMPTION_MANAGER_ROLES.includes(identity.roleCode))
        throw new Error("EXEMPTION_DIRECT_FORBIDDEN");
    if (!input.taskDate || input.taskDate !== formatBeijingDate(new Date()))
        throw new Error("EXEMPTION_TODAY_ONLY");
    ...
    if (!selectedOrgIds.length && !selectedUserIds.size)
        throw new Error("EXEMPTION_TARGET_REQUIRED");
    let managementScopePath = identity.scopePath ?? null;
    let baseOrg = null;
    if (["DEV_ADMIN", "HQ_ADMIN"].includes(identity.roleCode)) {
        if (!input.scopeOrgId)
            throw new Error("EXEMPTION_BASE_REQUIRED");
    ...
    for (const assignment of assignments) {
        const audience = await listAssignmentAudienceMembers(prisma, assignment, input.taskDate);
        for (const member of audience) {
            if (resolvedUsers.has(member.userId))
                continue;
            resolvedUsers.add(member.userId);
            const hallOrgPath = member.hallOrgPath;
            if (!hallOrgPath || !(hallOrgPath === managementScopePath || hallOrgPath.startsWith(`${managementScopePath}/`)))
                continue;
            const selectedByOrg = selectedOrgs.some((org) => hallOrgPath === org.path || hallOrgPath.startsWith(`${org.path}/`));
            if (!selectedByOrg && !selectedUserIds.has(member.userId))
                continue;
            targets.push({ assignment, member });
        }
    }
    const matchedUserIds = new Set(targets.map(({ member }) => member.userId));
    if (Array.from(selectedUserIds).some((userId) => !matchedUserIds.has(userId)))
        throw new Error("EXEMPTION_DIRECT_FORBIDDEN");
    return targets;
}
function ensureDailyRecordEditable(record, now = new Date()) {
    if (record.assignment?.category !== "DAILY")
        return;
    if (!record.recordDate)
        throw new Error("INVALID_RECORD_DATE");
    const context = getDailyTaskContext(now);
    if (record.recordDate === context.today) {
        if (record.assignment?.status !== "active")
            throw new Error("DAILY_RECORD_INACTIVE");
        return;
    }
...
function resolveNextRecordStatus(record, doneItems, now = new Date()) {
...
function itemRecordTextSnapshot(itemRecord) {
    if (!itemRecord?.answerText)
        return "";
...
async function buildManagerContributionSnapshot(recordId) {
    ...
    if (!enriched || !isManagerContributionRecord(enriched))
        return enriched;
    const identityIds = parseContributionLines((enriched.itemRecords ?? []).map((item) => item.answerText ?? "").join("\n")).map((entry) => entry.identityId);
    const identityLabelMap = await loadIdentityLabelMap(identityIds);
    return {
        ...enriched,
        itemContributionSummaries: (enriched.assignment?.template?.items ?? []).map((item) => {
            const itemRecord = (enriched.itemRecords ?? []).find((entry) => entry.taskItemId === item.id);
...
function ensureRecordReadyForSubmit(record) {
    if (getIncompleteRequiredItems(record).length > 0)
...
export const RecordService = {
    async getMyRecords(userId, identityId, scopePath, _roleCode) {
    ...
    async getRecord(recordId, userId, identityId) {
    ...
    async getById(recordId, userId, identityId) {
    ...
    async submitItemRecord(data) {
        ...
        if (record.exemption?.status === "approved")
            throw new Error("RECORD_EXEMPTED");
        const effectiveStatus = resolveTaskRecordStatus(record, now);
        if (effectiveStatus === "submitted" && !canSupplementSubmitted(record))
            throw new Error("RECORD_SUBMITTED");
        ...
        const itemRecord = await prisma.taskItemRecord.upsert({
            ...
            update: {
                ...
                answerOptions: data.answerOptions ? data.answerOptions : null,
    ...
    async submitRecord(recordId, userId, identityId) {
        const now = new Date();
        ...
        if (effectiveStatus === "submitted" && !canSupplementSubmitted(record))
            throw new Error("RECORD_SUBMITTED");
        ensureDailyRecordEditable(record, now);
        ensureRecordReadyForSubmit(record);
        const user = await prisma.user.findFirst({ where: { id: userId, status: "active" } });
        if (!user)
            throw new Error("USER_INACTIVE");
    ...
    async applyExemption(taskRecordId, userId, identityId, reason) {
        const normalizedReason = reason.trim();
        if (!normalizedReason)
            throw new Error("EXEMPTION_REASON_REQUIRED");
        ...
        if (recordAny.assignment?.category !== "DAILY" || !recordAny.recordDate)
            throw new Error("EXEMPTION_DAILY_ONLY");
        if (recordAny.status === "submitted")
            throw new Error("EXEMPTION_COMPLETED_FORBIDDEN");
        if (recordAny.exemption?.status === "pending" || recordAny.exemption?.status === "approved")
            throw new Error("EXEMPTION_EXISTS");
    ...
    async cancelExemption(taskRecordId, userId, identityId) {
        ...
        if (!["pending", "approved"].includes(existing.status))
            throw new Error("EXEMPTION_REVIEWED");
        await prisma.taskExemption.delete({ where: { id: existing.id } });
        return { cancelled: true, taskRecordId };
    },
    async reviewExemption(exemptionId, approved, reviewerId, identity) {
        ...
        if (exemption.status !== "pending")
            throw new Error("EXEMPTION_REVIEWED");
        if (exemption.taskRecord?.assignment?.category !== "DAILY")
            throw new Error("EXEMPTION_DAILY_ONLY");
        if (!canReviewDailyExemption(identity, exemption.taskRecord ?? {}))
            throw new Error("EXEMPTION_REVIEW_FORBIDDEN");
    ...
    async listExemptions(scopePath, roleCode, status) {
        const where = {};
        if (status)
            where.status = status;
    ...
    async directEnableExemptions(input, operatorUserId, identity) {
        ...
        for (const { assignment, member } of targets) {
            ...
            const requiredItemIds = new Set(assignment.template.items.filter((item) => item.isRequired !== false).map((item) => item.id));
            const completedRequired = record.itemRecords.filter((item) => requiredItemIds.has(item.taskItemId) && item.status === "done").length;
    ...
    async directDisableExemptions(input, identity) {
        const targets = await resolveDirectExemptionTargets(input, identity);
        let disabled = 0;
        let notEnabled = 0;
        for (const { assignment, member } of targets) {
    ...
    async reconfirmRecord(recordId, userId, identityId) {
        const record = await ensureVisibleRecord(recordId, userId, identityId, {
            assignment: { select: { category: true, status: true, preDeadlineConfirmEnabled: true } },
        });
        if (record.reconfirmStatus !== "pending")
            throw new Error("RECONFIRM_NOT_PENDING");
        const assignmentAny = record.assignment;
        if (!assignmentAny?.preDeadlineConfirmEnabled)
            throw new Error("RECONFIRM_NOT_PENDING");
        return prisma.taskRecord.update({
            where: { id: recordId },
            data: {
