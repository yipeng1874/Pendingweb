...
function findOversizedAnchorField(fields) {
    return fields.find((field) => field.value.length > field.maxLength);
}
function failIfAnchorFieldTooLong(res, fields) {
    const oversized = findOversizedAnchorField(fields);
    if (!oversized)
        return null;
    return fail(res, "ANCHOR_FIELD_TOO_LONG", `${oversized.label}不能超过 ${oversized.maxLength} 个字符，请勿粘贴抖音分享文案或完整链接`, 400);
}
function maskPhone(phone) {
    return phone.length === 11 ? `${phone.slice(0, 3)}****${phone.slice(-4)}` : "invalid-phone";
}
export const AnchorController = {
    async getRegisterOrgs(req, res) {
    ...
    async register(req, res) {
        ...
        if (!nickname || !phone || !password || !targetHallOrgId)
            return fail(res, "REGISTER_REQUIRED_FIELDS", "请填写账号昵称、手机号、密码并选择归属厅", 400);
        if (!/^\d{11}$/.test(phone))
            return fail(res, "PHONE_INVALID", "手机号必须为11位数字", 400);
        ...
        if (!hall)
            return fail(res, "HALL_NOT_FOUND", "选择的归属厅不存在或已停用", 400);
    ...
    async getProfileDetail(req, res) {
        ...
        if (!profile)
            return fail(res, "ANCHOR_PROFILE_NOT_FOUND", "主播档案不存在或无权查看", 404);
    ...
    async createProfile(req, res) {
        ...
        if (!nickname || !douyinUid || !hallOrgId)
            return fail(res, "ANCHOR_PROFILE_REQUIRED_FIELDS", "请填写主播昵称、抖音 UID 和归属厅", 400);
        const lengthError = failIfAnchorFieldTooLong(res, [
            { label: "昵称", value: nickname, maxLength: ANCHOR_TEXT_MAX_LENGTH },
            { label: "抖音号", value: douyinNo, maxLength: DOUYIN_NO_MAX_LENGTH },
            { label: "抖音 UID", value: douyinUid, maxLength: DOUYIN_UID_MAX_LENGTH },
        ...
        if (!hall)
            return fail(res, "HALL_NOT_FOUND", "归属厅不存在", 400);
        try {
    ...
    async updateProfile(req, res) {
        const allowed = ["nickname", "douyinNo", "douyinUid", "hallOrgId"];
    ...
    async migrateProfile(req, res) {
        const targetHallOrgId = text(req.body.targetHallOrgId);
        if (!targetHallOrgId)
            return fail(res, "HALL_NOT_FOUND", "请选择目标归属厅", 400);
        try {
    ...
    async getApplicationDetail(req, res) {
        ...
        if (!app)
            return fail(res, "APPLICATION_NOT_FOUND", "注册账号申请不存在或无权查看", 404);
        return ok(res, app);
    },
    async getCandidates(req, res) {
        const app = await prisma.anchorRegistrationApplication.findUnique({ where: { id: req.params.id } });
        if (!app)
            return fail(res, "APPLICATION_NOT_FOUND", "注册账号申请不存在", 404);
    ...
    async reviewApplication(req, res) {
        ...
        if (app.status !== "pending")
            return fail(res, "APPLICATION_REVIEWED", "该申请已审核", 400);
        if (!approved) {
        ...
        if (!douyinUid || !douyinNo || !nickname)
            return fail(res, "ANCHOR_REVIEW_REQUIRED_FIELDS", "审核通过必须填写账号昵称、抖音号和抖音 UID", 400);
        const lengthError = failIfAnchorFieldTooLong(res, [
            { label: "昵称", value: nickname, maxLength: ANCHOR_TEXT_MAX_LENGTH },
            { label: "抖音号", value: douyinNo, maxLength: DOUYIN_NO_MAX_LENGTH },
            { label: "抖音 UID", value: douyinUid, maxLength: DOUYIN_UID_MAX_LENGTH },
