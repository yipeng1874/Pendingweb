...
function buildAnchorConflictMessage(conflict) {
    const fields = [
...
function toTrimmedNullable(value) {
    const normalized = String(value ?? "").trim();
    return normalized || null;
}
function createAnchorBusinessError(code, message) {
...
function isAnchorUniqueConstraintError(error, field) {
...
async function assertNoAnchorRegistrationConflict(input) {
    ...
    if (douyinNo || douyinUid) {
        ...
        if (existingProfile) {
            ...
            error.code = existingProfile.douyinUid === douyinUid ? "DOUYIN_UID_EXISTS" : "DOUYIN_NO_EXISTS";
            throw error;
        }
        ...
        if (existingApplication) {
            ...
            error.code = existingApplication.douyinUid === douyinUid ? "DOUYIN_UID_EXISTS" : "DOUYIN_NO_EXISTS";
            throw error;
        }
    }
}
async function syncAnchorProfileSnapshots(tx, input) {
    const identities = await tx.userIdentity.findMany({
        where: { anchorProfileId: input.profileId, roleCode: "ANCHOR" },
        select: { id: true, userId: true },
    });
    const userIds = Array.from(new Set([input.boundUserId, ...identities.map((item) => item.userId)].filter(Boolean)));
    if (!userIds.length && !input.douyinNo && !input.douyinUid && !input.nickname)
        return;
    const taskRecordWhere = {
        OR: [
...
export const AnchorService = {
    async getRegisterOrgs(parentId, orgType, includeVirtual) {
        const where = {
            status: "active",
            ...(orgType ? { orgType: orgType } : { orgType: { in: ["BASE", "TEAM", "HALL"] } }),
            ...(parentId ? { parentId } : {}),
        };
    ...
    async getScopedOrgChildren(input) {
        ...
        const scopedRootWhere = !parent && input.identityOrgId && (input.roleCode === "BASE_ADMIN" || input.roleCode === "TEAM_ADMIN")
    ...
    async registerAnchor(nickname, phone, passwordHash, targetHallOrgId, douyinNo, douyinUid) {
    ...
    async exportProfiles(input) {
        ...
        const allPaths = [...new Set(profiles.map((p) => p.hallOrg?.path).filter(Boolean))];
        const ancestorOrgCodes = new Set();
        for (const path of allPaths) {
            const parts = path.split("/").filter(Boolean);
            if (parts[1])
                ancestorOrgCodes.add(parts[1]);
            if (parts[2])
                ancestorOrgCodes.add(parts[2]);
        }
    ...
    async createProfile(data) {
        ...
        if (!nickname)
            throw createAnchorBusinessError("ANCHOR_PROFILE_NICKNAME_REQUIRED", "主播昵称不能为空");
        if (!douyinUid)
            throw createAnchorBusinessError("ANCHOR_PROFILE_DOUYIN_UID_REQUIRED", "抖音 UID 不能为空");
        if (!hallOrgId)
            throw createAnchorBusinessError("HALL_NOT_FOUND", "归属厅不存在");
        const hall = await prisma.orgUnit.findFirst({ where: { id: hallOrgId, orgType: "HALL", status: "active" }, select: { id: true } });
        if (!hall)
            throw createAnchorBusinessError("HALL_NOT_FOUND", "归属厅不存在或已停用");
    ...
    async updateProfile(id, data) {
        return prisma.$transaction(async (tx) => {
            ...
            if (!current)
                throw createAnchorBusinessError("ANCHOR_PROFILE_NOT_FOUND", "主播档案不存在");
    ...
    async migrateProfile(id, input) {
        const targetHallOrgId = String(input.targetHallOrgId ?? "").trim();
        if (!targetHallOrgId)
            throw createAnchorBusinessError("HALL_NOT_FOUND", "目标归属厅不能为空");
        return prisma.$transaction(async (tx) => {
            const current = await tx.anchorProfile.findUnique({ where: { id } });
            if (!current)
                throw createAnchorBusinessError("ANCHOR_PROFILE_NOT_FOUND", "主播档案不存在");
            if (!current.boundUserId)
                throw createAnchorBusinessError("ANCHOR_PROFILE_MIGRATION_UNBOUND", "未绑定账号的主播档案不支持迁移");
            if (current.hallOrgId === targetHallOrgId)
                throw createAnchorBusinessError("ANCHOR_PROFILE_MIGRATION_SAME_HALL", "目标归属厅与当前归属厅一致，无需迁移");
            ...
            if (!targetHall)
                throw createAnchorBusinessError("HALL_NOT_FOUND", "目标归属厅不存在或已停用");
            if (!currentAnchorIdentity)
                throw createAnchorBusinessError("ANCHOR_PROFILE_MIGRATION_IDENTITY_REQUIRED", "当前主播身份未启用，无法迁移");
    ...
    async toggleProfileStatus(profile, enable) {
        return prisma.$transaction(async (tx) => {
            if (enable) {
    ...
    async deleteProfile(profile) {
        return prisma.$transaction(async (tx) => {
            ...
            const anchorIdentityIds = (await tx.userIdentity.findMany({
    ...
    async getApplications(input) {
        ...
        const teamIds = Array.from(new Set(apps.map((item) => item.hall?.parentId).filter(Boolean)));
        ...
        const baseIds = Array.from(new Set(teams.map((item) => item.parentId).filter(Boolean)));
    ...
    async getProfileDetail(profileId, scopePath, roleCode) {
    ...
    async getApplicationDetail(applicationId, scopePath, roleCode) {
    ...
    async getCandidates(app, keyword) {
    ...
    async rejectApplication(appId, reviewerId) {
        return prisma.anchorRegistrationApplication.update({
            where: { id: appId },
            data: { status: "rejected", reviewedBy: reviewerId, reviewedAt: new Date() }
        });
    },
    async approveApplication(app, douyinUid, douyinNo, nickname, _profileId, reviewerId) {
        await assertNoAnchorRegistrationConflict({
            phone: null,
            douyinNo,
            douyinUid,
