...
async function trimUserTasks(creatorId) {
...
export const ExportTaskService = {
    async createTask(input) {
        ...
        if (inProgress) {
            ...
            error.code = "EXPORT_TASK_IN_PROGRESS";
            throw error;
        }
    ...
    async getTaskFile(taskId, creatorId) {
        const task = await prisma.exportTask.findUnique({ where: { id: taskId } });
        if (!task) {
