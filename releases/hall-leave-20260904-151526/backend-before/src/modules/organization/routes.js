...
async function syncOrganizationRelations(tx, input) {
    ...
    if (input.orgType === "HALL") {
        ...
        const boundUserIds = Array.from(new Set(hallAnchorProfiles.map((item) => item.boundUserId).filter(Boolean)));
...
async function resolveOrgCode(tx, orgType, parent, body) {
    if (orgType === "TEAM") {
        if (!parent || parent.orgType !== "BASE")
            return { error: ["TEAM_PARENT_INVALID", "团队组织必须创建在基地下"] };
        return { orgCode: await generateTeamOrgCode(tx, parent.orgCode) };
    }
    if (orgType === "HALL") {
        if (!parent || parent.orgType !== "TEAM")
            return { error: ["HALL_PARENT_INVALID", "厅组织必须创建在团队下"] };
        const douyinUid = normalizeDouyinUid(body.douyinUid);
        if (!douyinUid)
            return { error: ["HALL_DOUYIN_UID_REQUIRED", "新建厅组织必须填写厅抖音 UID"] };
        return { orgCode: makeHallOrgCode(douyinUid), douyinUid };
    }
    const orgCode = text(body.orgCode).toUpperCase();
    if (!orgCode)
        return { error: ["ORG_CODE_REQUIRED", "请填写组织编码"] };
    return { orgCode };
}
...
async function assertHallIdentityUnique(input) {
    const or = [];
    if (input.douyinNo)
        or.push({ douyinNo: input.douyinNo });
    if (input.douyinUid)
        or.push({ douyinUid: input.douyinUid });
    if (!or.length)
        return null;
...
function buildHallDuplicateMessage(input, duplicated) {
    const conflicts = [];
    if (input.douyinNo && duplicated.douyinNo === input.douyinNo)
        conflicts.push(`厅抖音号 ${input.douyinNo}`);
    if (input.douyinUid && duplicated.douyinUid === input.douyinUid)
        conflicts.push(`厅抖音 UID ${input.douyinUid}`);
...
organizationRoutes.get("/orgs/children", permissionRequired("org:view"), async (req, res) => {
    ...
    const rootWhere = !parentId && !isAdmin
        ? req.identity?.orgId
            ? { id: req.identity.orgId, status: "active" }
            : scopePath
                ? { path: scopePath, status: "active" }
                : null
        : null;
    const nextOrgTypes = {
        ROOT: ["HQ", "BASE"],
            ...
            : req.identity?.roleCode === "BASE_ADMIN"
                ...
                : req.identity?.roleCode === "TEAM_ADMIN"
                    ...
                    : req.identity?.roleCode === "HALL_MANAGER"
                        ? ["HALL"]
                        : ["BASE", "TEAM", "HALL"];
    if (!rootWhere && !allowedTypes.length)
        return ok(res, []);
...
organizationRoutes.post("/orgs", permissionRequired("org:create"), async (req, res) => {
    ...
    if (!text(req.body.name))
        return fail(res, "ORG_NAME_REQUIRED", "请填写组织名称", 400);
    if (parentTypeMap[orgType] && parent?.orgType !== parentTypeMap[orgType])
        return fail(res, "ORG_PARENT_INVALID", "上级组织层级不符合规则", 400);
    ...
    if (orgType === "HALL") {
        ...
        if (missing.length)
            return fail(res, "HALL_REQUIRED_FIELDS_MISSING", `新建厅组织必须填写：${missing.join("、")}`, 400);
        const duplicated = await assertHallIdentityUnique({ douyinNo: normalizedDouyinNo, douyinUid: normalizedDouyinUid });
        if (duplicated)
            return fail(res, "HALL_IDENTITY_EXISTS", buildHallDuplicateMessage({ douyinNo: normalizedDouyinNo, douyinUid: normalizedDouyinUid }, duplicated), 409);
    }
    const createOrg = async () => {
        ...
        try {
            const org = await prisma.$transaction(async (tx) => {
                ...
                if ("error" in txResolved)
                    throw new Error(txResolved.error[1]);
            ...
            if (orgType === "TEAM" && isUniqueConstraintError(error, "org_code"))
                return { retry: true };
            if (isUniqueConstraintError(error, "douyin_no"))
                return { error: { code: "HALL_DOUYIN_NO_EXISTS", message: `厅抖音号 ${normalizedDouyinNo} 已存在` } };
            if (isUniqueConstraintError(error, "douyin_uid"))
                return { error: { code: "HALL_DOUYIN_UID_EXISTS", message: `厅抖音 UID ${douyinUid} 已存在` } };
            if (isUniqueConstraintError(error, "org_code"))
                return { error: { code: "ORG_CODE_EXISTS", message: `组织编码 ${orgCode} 已存在` } };
            throw error;
        }
    };
...
organizationRoutes.post("/orgs/halls/batch", permissionRequired("org:create"), async (req, res) => {
    ...
    if (!parent || parent.orgType !== "TEAM")
        return fail(res, "HALL_PARENT_INVALID", "批量新建厅必须先选择团队作为上级组织", 400);
    const writeScopePath = getWriteScopePath(req);
    ...
    if (!rows.length)
        return fail(res, "BATCH_ROWS_EMPTY", "请至少上传一条厅组织数据", 400);
    const seenDouyinNos = new Set();
    const seenDouyinUids = new Set();
    const errors = [];
    const normalizedRows = rows.map((row, index) => {
        ...
        if (missing.length)
            errors.push({ row: index + 1, message: `缺少必填字段：${missing.join("、")}` });
        if (douyinNo && seenDouyinNos.has(douyinNo))
            errors.push({ row: index + 1, message: `厅抖音号 ${douyinNo} 在表格内重复` });
        if (douyinUid && seenDouyinUids.has(douyinUid))
            errors.push({ row: index + 1, message: `厅抖音 UID ${douyinUid} 在表格内重复` });
        if (douyinNo)
            seenDouyinNos.add(douyinNo);
        if (douyinUid)
            seenDouyinUids.add(douyinUid);
        return { row, douyinNo, douyinUid, orgCode };
    });
    if (errors.length)
        return fail(res, "BATCH_VALIDATION_FAILED", errors.map((item) => `第 ${item.row} 行${item.message}`).join("；"), 400);
    ...
    if (existingNos.length)
        return fail(res, "HALL_DOUYIN_NO_EXISTS", `以下厅抖音号已存在：${existingNos.map((item) => item.douyinNo).join("、")}`, 409);
    if (existingUids.length)
        return fail(res, "HALL_DOUYIN_UID_EXISTS", `以下厅抖音 UID 已存在：${existingUids.map((item) => item.douyinUid).join("、")}`, 409);
    if (existingCodes.length)
        return fail(res, "ORG_CODE_EXISTS", `以下组织编码已存在：${existingCodes.map((item) => item.orgCode).join("、")}`, 409);
    try {
        ...
        if (isUniqueConstraintError(error, "douyin_no"))
            return fail(res, "HALL_DOUYIN_NO_EXISTS", "批量创建失败：存在重复的厅抖音号", 409);
        if (isUniqueConstraintError(error, "douyin_uid"))
            return fail(res, "HALL_DOUYIN_UID_EXISTS", "批量创建失败：存在重复的厅抖音 UID", 409);
        if (isUniqueConstraintError(error, "org_code"))
            return fail(res, "ORG_CODE_EXISTS", "批量创建失败：存在重复的组织编码", 409);
        throw error;
    }
});
organizationRoutes.patch("/orgs/:id", permissionRequired("org:update"), async (req, res) => {
    ...
    if (org.orgType === "HALL") {
        ...
        if (!nextDouyinNo)
            return fail(res, "HALL_DOUYIN_NO_REQUIRED", "厅抖音号不能为空", 400);
        if (!nextDouyinUid)
            return fail(res, "HALL_DOUYIN_UID_REQUIRED", "厅抖音 UID 不能为空", 400);
        const duplicated = await assertHallIdentityUnique({ douyinNo: nextDouyinNo, douyinUid: nextDouyinUid, excludeOrgId: org.id });
        if (duplicated)
            return fail(res, "HALL_IDENTITY_EXISTS", buildHallDuplicateMessage({ douyinNo: nextDouyinNo, douyinUid: nextDouyinUid }, duplicated), 409);
        ...
        try {
            ...
            if (isUniqueConstraintError(error, "douyin_no"))
                return fail(res, "HALL_DOUYIN_NO_EXISTS", `厅抖音号 ${nextDouyinNo} 已存在`, 409);
            if (isUniqueConstraintError(error, "douyin_uid") || isUniqueConstraintError(error, "org_code"))
                return fail(res, "HALL_DOUYIN_UID_EXISTS", `厅抖音 UID ${nextDouyinUid} 已存在`, 409);
            throw error;
        }
    }
...
organizationRoutes.post("/orgs/:id/move", permissionRequired("org:update"), async (req, res) => {
    ...
    if (!org || !newParent)
        return fail(res, "ORG_NOT_FOUND", "组织或新上级不存在", 404);
    const writeScopePath = getWriteScopePath(req);
    if (writeScopePath) {
        ...
        if (!orgInScope)
            return fail(res, "FORBIDDEN", "无权迁移该组织", 403);
        if (!newParentInScope)
            return fail(res, "FORBIDDEN", "无权迁移到该上级组织", 403);
    }
    if (org.orgType === "HQ")
        return fail(res, "ORG_MOVE_FORBIDDEN", "公司总部不能迁移", 400);
    if (newParent.path.startsWith(`${org.path}/`) || newParent.id === org.id)
        return fail(res, "ORG_MOVE_INVALID", "不能迁移到自身或下级组织", 400);
    const expectedParentType = org.orgType === "BASE" ? "HQ" : org.orgType === "TEAM" ? "BASE" : org.orgType === "HALL" ? "TEAM" : undefined;
    if (newParent.orgType !== expectedParentType)
        return fail(res, "ORG_MOVE_INVALID_PARENT", "新上级层级不符合组织规则", 400);
    const oldPath = org.path;
...
organizationRoutes.delete("/orgs/:id", permissionRequired("org:update"), async (req, res) => {
    ...
    if (!org)
        return fail(res, "ORG_NOT_FOUND", "组织不存在", 404);
    if (org.orgType === "HQ")
        return fail(res, "ORG_DELETE_FORBIDDEN", "公司总部不能删除", 400);
    const writeScopePath = getWriteScopePath(req);
    if (writeScopePath && !isAncestorPath(writeScopePath, org.path) && org.path !== writeScopePath) {
    ...
    const [children, identities, anchors, taskTemplates, assignmentTargets, taskRecords, feishuConfigs, registrationApplications] = await Promise.all([
        ...
        (prisma.feishuEnterpriseConfig?.count
            ? prisma.feishuEnterpriseConfig.count({ where: { OR: [{ baseOrgId: org.id }, { teamOrgId: org.id }] } })
            : Promise.resolve(0)),
        prisma.anchorRegistrationApplication.count({ where: { targetHallOrgId: org.id } }),
    ]);
    const blockedReasons = [];
    if (children)
        blockedReasons.push(`下级组织 ${children} 条`);
    if (identities)
        blockedReasons.push(`身份数据 ${identities} 条`);
    if (anchors)
        blockedReasons.push(`主播数据 ${anchors} 条`);
    if (taskTemplates)
        blockedReasons.push(`任务模板 ${taskTemplates} 条`);
    if (assignmentTargets)
        blockedReasons.push(`任务分配目标 ${assignmentTargets} 条`);
    if (taskRecords)
        blockedReasons.push(`任务记录 ${taskRecords} 条`);
    if (feishuConfigs)
        blockedReasons.push(`飞书企业配置 ${feishuConfigs} 条`);
    if (registrationApplications)
        blockedReasons.push(`主播注册申请 ${registrationApplications} 条`);
...
organizationRoutes.get("/org/feishu-team-options", permissionRequired("org:view"), async (req, res) => {
    ...
    if (!baseOrgId)
        return fail(res, "FEISHU_CONFIG_BASE_REQUIRED", "请选择基地后再加载团队", 400);
    const scopePath = req.identity?.roleCode !== "DEV_ADMIN" ? req.identity?.scopePath ?? undefined : undefined;
...
organizationRoutes.get("/org/feishu-configs", permissionRequired("org:view"), async (req, res) => {
    const feishuConfigModel = prisma.feishuEnterpriseConfig;
...
organizationRoutes.post("/org/feishu-configs", permissionRequired("org:update"), async (req, res) => {
    ...
    if (!name)
        return fail(res, "FEISHU_CONFIG_NAME_REQUIRED", "请填写飞书企业名称", 400);
    if (!appId)
        return fail(res, "FEISHU_CONFIG_APP_ID_REQUIRED", "请填写 App ID", 400);
    if (!appSecret)
        return fail(res, "FEISHU_CONFIG_APP_SECRET_REQUIRED", "请填写 App Secret", 400);
    if (!env.FEISHU_REDIRECT_URI)
        return fail(res, "FEISHU_CONFIG_REDIRECT_URI_REQUIRED", "系统未配置统一回调地址，请先设置 FEISHU_REDIRECT_URI", 400);
    if (!baseOrgId)
        return fail(res, "FEISHU_CONFIG_BASE_REQUIRED", "请选择基地", 400);
    if (!teamOrgId)
        return fail(res, "FEISHU_CONFIG_TEAM_REQUIRED", "请选择团队", 400);
    ...
    if (!baseOrg || baseOrg.orgType !== "BASE")
        return fail(res, "FEISHU_CONFIG_BASE_INVALID", "所选基地不存在", 400);
    if (!teamOrg || teamOrg.orgType !== "TEAM")
        return fail(res, "FEISHU_CONFIG_TEAM_INVALID", "所选团队不存在", 400);
    if (teamOrg.parentId !== baseOrg.id)
        return fail(res, "FEISHU_CONFIG_ORG_MISMATCH", "所选团队不属于当前基地", 400);
    try {
        ...
        if (isUniqueConstraintError(error, "app_id"))
            return fail(res, "FEISHU_CONFIG_APP_ID_EXISTS", `App ID ${appId} 已存在`, 409);
...
organizationRoutes.patch("/org/feishu-configs/:id", permissionRequired("org:update"), async (req, res) => {
    ...
    if (!existed)
        return fail(res, "FEISHU_CONFIG_NOT_FOUND", "飞书企业配置不存在", 404);
...
organizationRoutes.patch("/org/feishu-configs/:id/status", permissionRequired("org:update"), async (req, res) => {
    ...
    if (!["active", "paused"].includes(status))
        return fail(res, "FEISHU_CONFIG_STATUS_INVALID", "状态仅支持 active 或 paused", 400);
    const existed = await feishuConfigModel.findUnique({ where: { id: req.params.id } });
    if (!existed)
        return fail(res, "FEISHU_CONFIG_NOT_FOUND", "飞书企业配置不存在", 404);
