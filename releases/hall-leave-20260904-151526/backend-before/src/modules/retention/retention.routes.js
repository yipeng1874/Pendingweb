...
retentionRoutes.post("/retention/upsert", permissionRequired("task:report:view"), async (req, res) => {
    ...
    const { teamOrgId, teamOrgName, recordMonth, loss3Days, loss15Days, loss30Days, activeCount, } = req.body ?? {};
    ...
    if (loss3Days == null || loss15Days == null || loss30Days == null ||
        activeCount == null) {
...
retentionRoutes.get("/retention/list", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
...
retentionRoutes.get("/retention/by-month", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
