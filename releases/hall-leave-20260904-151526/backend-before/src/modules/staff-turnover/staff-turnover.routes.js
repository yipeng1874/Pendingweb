...
staffTurnoverRoutes.post("/staff-turnover/upsert", permissionRequired("task:report:view"), async (req, res) => {
    ...
    const { teamOrgId, teamOrgName, recordDate, lossOnlineCount, lossOnlineAvgWave, lossOfflineCount, lossOfflineAvgWave, activeOnlineCount, activeOnlineAvgWave, activeOfflineCount, activeOfflineAvgWave, } = req.body ?? {};
    ...
    if (lossOnlineCount == null || lossOnlineAvgWave == null ||
...
staffTurnoverRoutes.get("/staff-turnover/list", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
...
staffTurnoverRoutes.get("/staff-turnover/by-date", permissionRequired("task:report:view"), async (req, res) => {
    let baseOrg;
    try {
        baseOrg = await resolveBaseScopeOrg(req.query.scopeOrgId, req.identity);
    }
