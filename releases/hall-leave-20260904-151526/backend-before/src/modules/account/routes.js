...
async function resolveScopedSearchPath(req, scopeOrgId) {
    if (!scopeOrgId) {
        if (req.identity?.roleCode === "DEV_ADMIN")
            return undefined;
...
async function resolvePrimaryAnchorProfiles(tx, userIds) {
    if (!userIds.length)
        return new Map();
    ...
    for (const identity of identities) {
        if (!identity.anchorProfile || map.has(identity.userId))
            continue;
        map.set(identity.userId, identity.anchorProfile);
    }
    return map;
}
function ensureDevAdmin(req) {
    return req.identity?.roleCode === "DEV_ADMIN";
}
function isManagementRole(roleCode) {
    return roleCode !== "ANCHOR";
}
const orgTypeRoleMap = {
    HQ: "HQ_ADMIN",
    BASE: "BASE_ADMIN",
    TEAM: "TEAM_ADMIN",
    HALL: "HALL_MANAGER",
};
function isRoleAllowedForOrg(roleCode, orgType) {
...
async function grantIdentity(tx, input) {
...
async function findManagementScopeConflicts(userId, org, ignoreIdentityId) {
...
accountRoutes.get("/accounts/search", permissionRequired("account:view"), async (req, res) => {
    ...
    try {
        ...
        if (error.message === "SCOPE_ORG_FORBIDDEN")
            return fail(res, "SCOPE_ORG_FORBIDDEN", "当前身份无权在该基地范围内搜索账号", 403);
        throw error;
    }
});
accountRoutes.get("/accounts/:id/detail", permissionRequired("account:view"), async (req, res) => {
    ...
    if (req.identity?.scopePath && req.identity.roleCode !== "DEV_ADMIN") {
        ...
        if (!visible)
            return fail(res, "ACCOUNT_FORBIDDEN", "当前身份无权查看该账号", 403);
    }
...
accountRoutes.post("/accounts/:id/identities", permissionRequired("identity:grant"), async (req, res) => {
    ...
    if (isManagementRole(roleCode)) {
        if (!org)
            return fail(res, "ORG_REQUIRED", "管理身份必须选择组织", 400);
    ...
    if (roleCode === "ANCHOR" && !anchorProfile)
        return fail(res, "ANCHOR_PROFILE_REQUIRED", "主播身份必须选择主播档案", 400);
...
accountRoutes.patch("/accounts/identities/:id/status", permissionRequired("identity:grant"), async (req, res) => {
    ...
    if (!current)
        return fail(res, "IDENTITY_NOT_FOUND", "身份不存在", 404);
...
accountRoutes.delete("/accounts/:id", async (req, res) => {
    if (!ensureDevAdmin(req))
        return fail(res, "FORBIDDEN", "仅总公司开发管理员可强制删除账号", 403);
...
accountRoutes.delete("/accounts/identities/:id", async (req, res) => {
    if (!ensureDevAdmin(req))
        return fail(res, "FORBIDDEN", "仅总公司开发管理员可强制删除身份", 403);
