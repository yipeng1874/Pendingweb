...
async function ensureTaskExecutionPermissions() {
    ...
    const data = requiredTaskPermissions.flatMap((permission) => permission.roleCodes
...
export async function ensureTaskAssignmentSchemaCompatibility() {
  ...
  `;
    ...
    if (Number(tableRow?.count ?? 0) === 0) {
        ...
        if (dailyNotifyScheduleState.createdTable || addedRolePermissions.length) {
            console.log(`[db] 已补齐任务模块兼容结构：自动通知表=${dailyNotifyScheduleState.createdTable ? "已创建" : "无"}；角色权限=${addedRolePermissions.join(",") || "无"}`);
        }
        return {
            addedColumns: [],
            addedIndexes: [],
  ...
  `;
    ...
    for (const column of requiredColumns) {
        ...
        await prisma.$executeRawUnsafe(`ALTER TABLE \`task_assignments\` ADD COLUMN \`${column.columnName}\` ${column.definition}${afterClause}`);
  ...
  `;
    ...
    for (const index of requiredIndexes) {
        if (existingIndexes.has(index.indexName))
            continue;
        await prisma.$executeRawUnsafe(`ALTER TABLE \`task_assignments\` ADD INDEX \`${index.indexName}\` ${index.definition}`);
    ...
    if (addedColumns.length || addedIndexes.length || addedRolePermissions.length || dailyNotifyScheduleState.createdTable) {
        console.log(`[db] 已补齐任务兼容结构：列=${addedColumns.join(",") || "无"}；索引=${addedIndexes.join(",") || "无"}；自动通知表=${dailyNotifyScheduleState.createdTable ? "已创建" : "无"}；角色权限=${addedRolePermissions.join(",") || "无"}`);
    }
    return {
        addedColumns,
        addedIndexes,
