...
export function WorkflowCard({ task, currentUserId, onUpdate }: { task: WorkflowTask; currentUserId?: string; onUpdate: (task: WorkflowTask) => void }) {
  ...
  return <div className={`dashboard-task-card${active ? " dashboard-task-active" : ""}`}>
    <button className="dashboard-card-toggle" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}><div className="dashboard-card-main"><div className="dashboard-card-topline"><span className="tag tag-purple">流转任务</span><span className={`tag ${done ? "tag-green" : active ? "task-status-active" : status === "已逾期" ? "tag-red" : "tag-slate"}`}>{status}</span></div><p className="todo-title">{task.title}</p><div className="dashboard-meta"><span>{task.createdByName} · {task.targetOrgName}</span><span>截止 {task.dueAt ? new Date(task.dueAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) : "不限"}</span></div></div>{expanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</button>
    {expanded ? <div className="dashboard-expanded"><p className="dashboard-description dashboard-description-full">{task.description}</p><p className="section-note">填写自己的节点，逐题确认；全部必填题确认后自动完成节点。</p>{task.steps.map((step) => {
      const reason = workflowEditingReason(task, step, currentUserId);
      return <section className="collaboration-step" key={step.id}><div className="dashboard-card-topline"><strong>{step.order}. {step.title}</strong><span className={`tag ${step.status === "completed" ? "tag-green" : "tag-blue"}`}>{step.status === "completed" ? "已完成" : step.assigneeUserId === currentUserId ? "我的节点" : "他人节点"}</span></div><div className="section-note">负责人：{step.assigneeName || "未指定"}</div>{step.requirement ? <p className="dashboard-description dashboard-description-full">{step.requirement}</p> : null}{reason ? <p className="section-note">{reason}</p> : null}<div className="list">{step.questions.map((question, index) => <QuestionEditor taskSaving={taskSave.saving} key={question.id} question={question} order={index + 1} answer={step.stepAnswers?.find((answer) => answer.questionId === question.id)} lockedReason={() => workflowEditingReason(task, step, currentUserId)} onSave={async (answer) => { await taskSave.save(async () => (await taskApi.saveWorkflowAnswer(task.id, step.id, answer)).task); }} />)}</div>{!step.questions.length && !reason ? <button className="btn btn-primary" disabled={Boolean(busyStep) || taskSave.saving} onClick={async () => { if (busyStep || workflowEditingReason(task, step, currentUserId)) return; setBusyStep(step.id); setError(""); try { await taskSave.save(() => taskApi.submitWorkflowStep(task.id, step.id)); } catch (err) { setError(err instanceof Error ? err.message : "提交失败"); } finally { setBusyStep(""); } }}>确认完成节点</button> : null}</section>;
